...
#define MKEY( field, value )   field  = atoi(value);
#define MSKEY( field, value )  if (field!=NULL) free_string(field);field = str_dup(value);
...
struct coordinate_data
{
    ...
    int yco; /* Y plane */
    int zco; /* Z plane */
    unsigned long int pco; /* Number of plane. (when you 'enter' a door, it's a new plane) */
    /* Last one is unsigned, because planes can't go into negatives.
     */
};
...
struct clan_data
{
    ...
    char    *   leader;                 /* Clan leader */
    char    *   enemies;		/* Enemies (if any) */
    char    *   members;            /* Clan Members */
    int		eq[MAX_CLAN_EQ];	/* vnums of objects to load */
    int         clan_numb;          /* Clan Number */
...
struct  class_type
{
    ...
    sh_int      toremort;               /* What class to remort to?     */
    sh_int      fromremort;             /* Accept what classes?         */
    bool        isremort;               /* Is this a remort Class?      */
    bool        kingdom[5];             /* What kingdom?                */
    char     *  cdesc;                  /* Class Description            */
};

struct  remort_class_type
{
    char        who_name        [4];    /* Three-letter name for 'who'  */
    char     *  class_name;             /* Full name                    */
    sh_int      attr_prime;             /* Prime attribute              */
    ...
    char     *  skill1;			/* Auto-learnt skill if any     */
    sh_int      toremort;               /* What class to remort to?     */
    sh_int      fromremort;             /* What class to remort from?   */
};
...
struct race_type
{
    ...
    char    *   race_coltitle;          /* A colored name */
    ...
    int         suscept_realms;
    bool        player_allowed;
    bool        kingdom[5];            /* Kingdom allowed? */
    char   *    racedesc[5];
    bool        isremort;              /* Remort race? */
};
...
struct  mob_index_data
{
    ...
    sh_int		position;
    int			hunt_flags;
    sh_int      hometown; /* for guards */
    sh_int      report_room; /* For scouts, room to report back to */
    sh_int      chance_of_item; /* Percentile chance a random item will even be on him on repop? */
};
...
struct  char_data
{
     ...
     */
    CHAR_DATA *    spotted;
	ROOM_INDEX_DATA * was_in_room_guard;
    ROOM_INDEX_DATA * hunt_dest;
    bool going_to_town;
    bool going_to_post;
...
/*    CHAR_DATA *         hunting;
    ...
    sh_int              position;
    sh_int              practice;
    sh_int              train;          /* For training stats */
    int			num_of_spells;
    int                 num_of_skills;
    float              carry_weight;
    sh_int              carry_number;
...
/* Stuff to handle automatic quests for players */
    ...
    int			intell_exp;
    sh_int      report_room; /* Room to report to if is scout! */
    int         steal_vnum;  /* Random Item to steal, 0 if already stolen or none */
};
...
struct  pc_data
{
    ...
    int		move_from_gain;
    long        imc_deaf;    /* IMC channel flags */
    long        imc_allow;
    long        imc_deny;
    ...
    bool valid_email;
    char * assist_msg;
    sh_int     points; /* How many CP they have, the more they have, the more XP to level. */
    bool       group[MAX_GROUP]; /* If they have a group or not */
    bool       autobuild;          /* Auto dig in the builder by typing a direction. */
    sh_int     strategy;           /* Strategy for fighting */
    int        laston;             /* = current_time at logoff */
	sh_int     learned_str;        // The amount of skills learned in
	sh_int     learned_int;        // a particular attribute.
	sh_int     learned_wis;
	sh_int     learned_dex;
	sh_int     learned_con;
	sh_int     learned_lck;
...
struct  obj_data
{
    ...
    MONEY_TYPE * money;
    bool               stored; /* Is it stored or not? */
    bool               sat;    /* Is it being sat on? */
    bool                trapped; /* Used for ITEM_RANDOM_CHEST */
	int                 stored_order;
...
struct  exit_data
{
    ...
    char *              keyword;
    char *              description;
    bool                is_named; /* If it has a named exit, like 'enter bush' */
    bool                seperate_plane; /* Is it considered it's own 'mini' area with new coordinates? */
};
...
/* MAG mod */
    ...
    sh_int		reset_rate;
    char *		reset_msg;
    bool        linked_main; /* Linked to 'main' map? */
    bool        being_mapped; /* Is it currentlly being mapped by the coordinate mapper? */
};
...
/* -S- mod... don't save this with OLC :P */
    ...
    MONEY_TYPE * treasure;
    int         xco;       /* Maps are now logical. Coordinates are stored with every room. */
    int         yco;       /* This of course assumes that there won't be any more than */
    int         zco;       /* 4,294,967,295 per coordinate. Yes that's the real limit. */
   unsigned long int pco;       /* Unsigned because it can't go into negatives. */
    bool        checked;   /* Was already ran through the map matrix? Unsaved variable, set to false on load. */
    bool        is_linked_main; /*Linked to the main word?*/
...
struct  skill_type
{
    ...
    sh_int      prime_att;              /* Prime Attribute for the spell/skill */
	sh_int      apt_needed;             /* Aptitude needed in prime attribute to gain skill */
	sh_int      famil;                  /* How many skills/spells of that attrib must you know? */
...
struct  group_type
{
    ...
    sh_int      points[MAX_CLASS];      /* How many points?             */
    char *      spells[20];             /* What spells are in group?    */
};
...
char    *get_pkn         args( ( CHAR_DATA *ch ) );
...
int     class_lookup    args( ( const char *class_name ) );
int     rclass_lookup    args( ( const char *class_name ) );
int     classt_lookup    args( ( const char *class_name, CHAR_DATA *ch ) );
...
bool    can_use_skill   args( ( CHAR_DATA *ch, int skill ) );
bool    can_see_sobj    args( ( CHAR_DATA *ch, OBJ_DATA *obj ) );
OBJ_DATA *get_obj_stored args( ( CHAR_DATA *ch, char *argument ) );
void    spsk_num        args( ( CHAR_DATA *ch ) );
long    number2_mm      args( ( void ) );
int     number2_bits    args( ( int width ) );
int     get_skill       args( ( CHAR_DATA *ch, int sn ) );
bool    has_spell       args( ( CHAR_DATA *ch, int sn ) );
void    check_improve   args( ( CHAR_DATA * ch, int sn, bool success, int multiplier ) );
char * get_clan_info args ( ( int num, int which ) );
int    get_clan_numin args( ( int num, int which ) );
int    get_clan_eq    args( ( int num, int which ) );
char *one_argument_st args ( (char *argument, char *arg_first) );
char *get_title args ( ( CHAR_DATA *ch ) );
char *get_title_nocol args ( ( CHAR_DATA *ch ) );
void smash_return args ( ( char *str ) );
CHAR_DATA *get_char_connected args (( CHAR_DATA *ch, char *argument ));
void save_clans args ( ( void ) );
bool num_infix args( ( const char *astr, const char *bstr ) );
void fix_enemies args( (CLAN_DATA *clan) );
char * num_mod args (( char * mod_string,  char *argument ));
void fread_clan args( ( FILE *fp ) );
void raw_delclan args(( CHAR_DATA *ch, char *argument ));
void fix_hack args ( (CLAN_DATA *clan ));
bool can_gain_skill args ( ( CHAR_DATA *ch, int sn ) );
void update_scale args ( (CHAR_DATA *ch, int sn, bool down ) );
char *quote args (( char *argument));

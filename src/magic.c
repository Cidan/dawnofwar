...
//    char msg2 [MAX_STRING_LENGTH];
    ...
    for ( rch = ch->in_room->first_person; rch; rch = rch->next_in_room )
    {
      ...
      continue;
         ...
         if ( !IS_NPC( rch ) && IS_SET( rch->pcdata->pflags, PFLAG_BLIND_PLAYER ) )
          {
    sprintf( buf,  "Someone utters the words, '%s'.\n\r",skill_table[sn].name );
    send_to_char( buf, rch);
         } else {
    sprintf( buf,  "%s utters the words, '%s'.\n\r",PERSON(rch,ch), skill_table[sn].name );
    send_to_char( buf, rch);
              }
    }
...
bool saves_spell( int level, CHAR_DATA *victim )
{
    ...
    save = 40 +
     ( ( get_psuedo_level( victim ) > 60 ?
       ...
       - level
       - URANGE( -40, victim->saving_throw, 40 ) );
    ...
    if (number_percent( ) < save)
     saved = TRUE;
#if 0
    sprintf( log_buf, "%s level %d wismod %d savemod %d save total %d against level %d, %s ).",
      victim->name, get_psuedo_level( victim ),
      wis_app[get_curr_wis( victim )].spell_save,
      victim->saving_throw, save,
      level,
      ( saved ? "{CSAVED{x" : "{RFAILED{x" ) );
...
*/
    ...
    {
       send_to_char( "You don't have that spell.\n\r", ch );
       return;
    }
    ...
    if ( !char_login && IS_SET( ch->in_room->room_flags, ROOM_NO_MAGIC ) 
       && ( skill_table[sn].flag2 != WOLF ) && !IS_IMMORTAL ( ch ) )
    {
       send_to_char( "An anti-magic field prevents you from casting!\n\r", ch );
       return;
    }
...
/*
    if (  ( best == -1 )
       ...
       || (  ( skill_table[sn].flag2 == WOLF )
          && ( !IS_WOLF( ch ) ) )   )
    {
	send_to_char( "You can't do that lalal.\n\r", ch );
	return;
    }
*/  
...
*/
    ...
    if (  ( skill_table[sn].flag2 == NORM )
       && ( is_affected( ch, skill_lookup( "mystical focus" ) )  )  )
    {
      mana *= 2.5;
    }
    ...
    if ( IS_VAMP( ch ) && ( skill_table[sn].flag2 == VAMP ) )
    {
        ...
        {
            send_to_char( "{xYou don't have enough {Rblood{x to cast that spell.\n\r", ch );
            return;
        } 
        else;
    }
    ...
    if ( skill_table[sn].flag2 == WOLF 
       && ( !IS_NPC( ch ) && IS_WOLF( ch ) )
       && ch->pcdata->bloodlust < mana  )
    {
      send_to_char( "{yYou aren't able to summon enough {GRAGE{x!\n\r", ch );
      return;
    }
      else if (  ch->mana < mana && !IS_IMMORTAL(ch) )
      {
    ...
    if ( !char_login && !IS_NPC(ch) 
       && number_percent( ) > cast_chance )
    {
      bool good_cast = FALSE;

	  if (IS_IMMORTAL(ch) || ch->pcdata->learned[sn] >= 101)
		  good_cast = TRUE;
      ...
      if ( !good_cast )
      {
        ...
        else if ( skill_table[sn].flag2 == WOLF
                && ( !IS_NPC( ch ) && IS_WOLF( ch ) )  )
...
{
	if (victim == ch)
	    send_to_char("Your skin is already hard.\n\r", ch);
	else
	    act("$N's skin is already hard.", ch, NULL, victim, TO_CHAR);
	return TRUE;
    }
...
bool spell_blindness( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
	...
	{
	if (victim == ch)
	    send_to_char("You are already blinded...?!\n\r", ch);
	else
...
bool spell_charm_person( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
	...
	{
	if (victim == ch)
	    send_to_char("You shouldn't be seeing this!\n\r", ch);
	else
	    act("$N's loves someone else!", ch, NULL, victim, TO_CHAR);
	return TRUE;
    }
...
bool spell_cure_blindness( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
	...
	{
	if (victim == ch)
	    send_to_char("You aren't blinded!\n\r", ch);
	else
	    act("$N isn't blind!", ch, NULL, victim, TO_CHAR);
	return TRUE;
    }
...
bool spell_cure_poison( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
	...
	{
	if (victim == ch)
	    send_to_char("You aren't {gpoisoned{x.\n\r", ch);
	else
	    act("$N isn't {gpoisoned{x.", ch, NULL, victim, TO_CHAR);
	return TRUE;
    }
...
bool spell_detect_evil( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
	...
	{
	if (victim == ch)
	    send_to_char("You can already detect evil.\n\r", ch);
	else
...
bool spell_detect_hidden( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
	...
	{
	if (victim == ch)
	    send_to_char("You can already see hidden.\n\r", ch);
	else
	    act("$N can already see hidden.", ch, NULL, victim, TO_CHAR);
	return TRUE;
    }
    ...
    if ( ch != victim )
      act("$N seems more aware.", ch, NULL, victim, TO_CHAR);
    return TRUE;
}
...
bool spell_detect_invis( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
	...
	{
	if (victim == ch)
	    send_to_char("You can already see invisible things.\n\r", ch);
	else
...
bool spell_dispel_magic( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
   ...
   {
     send_to_char( "You are too {GENRAGED {xTo cast spells!\n\r", ch );
     return FALSE;
   }
   ...
   {
      if ( IS_NPC(victim) )         chance = 100;
      else                       chance = 75;
	
      if ( ch == victim )   chance = 100;
      ...
      if (  ( IS_AFFECTED( victim, AFF_CLOAK_REFLECTION ) )
         && ( ch != victim )
         && ( number_percent( ) < ( victim->level - 50 ) )  )
      {
            
        act( "{x$n's {Bc{Rl{Go{Ya{Ck{x glows brightly as $N's spell hits it{x!!", victim, NULL, ch, TO_ROOM );
        act( "{x$N's {Bc{Rl{Go{Ya{Ck{x glows brightly{x!!", ch, NULL, victim, TO_CHAR );
        act( "{xYour {Bc{Rl{Go{Ya{Ck{x glows brightly{x!!!", victim, NULL, ch, TO_CHAR );
        CREF( vch_next, CHAR_NEXTROOM );
        for ( vch = ch->in_room->first_person; vch; vch = vch_next )
        {
          vch_next = vch->next_in_room;
            ...
            && ( number_percent( ) < ( victim->level - 40 ) )  )
    {
      ...
      act( "{x$n's {Bcloak{x glows brightly as $N's spell hits it, then fades{x!!", victim, NULL, ch, TO_ROOM );
      act( "{x$N's {Bcloak{x glows brightly, and absorbs your spell{x!!", ch, NULL, victim, TO_CHAR );
      act( "{xYour {Bcloak{x glows brightly, and absorbs $N's spell{x!!!", victim, NULL, ch, TO_CHAR );
      CREF( vch_next, CHAR_NEXTROOM );
      ...
      || ( victim == ch->fighting ) )
   {
     switch( paf->bitvector )
     {
       ...
       case AFF_CHARM :
       case AFF_HOLD :
       case AFF_PARALYSIS : continue; break;
       default : break;
     }
   }
	 ...
	 {
          chance = ( chance ) / 3;
	    act( "{xThe {Bcloak{x around $n is ripped to shreds!", victim, NULL, victim, TO_ROOM );
	    send_to_char( "{xYour {Bcloak{x is ripped from your body!\n\r", victim );
	 ...
	 {
          chance = ( chance ) / 3;
	    act( "{xThe {Bc{Rl{Go{Ya{Ck{x around $n is ripped to shreds!", victim, NULL, NULL, TO_ROOM );
	    send_to_char( "{xYour {Bc{Rl{Go{Ya{Ck{x is ripped from your body!\n\r", victim );
...
/*	 if ( IS_AFFECTED( victim, AFF_INFRARED ) )
       ...
       if ( IS_AFFECTED( victim, AFF_CLOAK_FLAMING ) && ( number_percent() < chance ) )
	 {
	    ...
	    act( "{xThe {Rcloak{x around $n is ripped to shreds!", victim, NULL, NULL, TO_ROOM );
	    send_to_char( "{xYour {Rcloak{x is ripped from your body!\n\r", victim );
...
bool spell_identify( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
    ...
    sprintf( buf,
	"{xObject '%s' is {Rtype{x %s, {Cextra flags{x %s.\n\r{mWorn{x: %s, {cWeight{x: %d, {Yvalue{x: %s {x, {Glevel{x: %d.\n\r",

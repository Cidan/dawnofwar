...
const struct lookup_type tab_attack_types[] =
{
   { "Exotic",     0 , 10},
   { "Sword",   1 , 10},
   { "Dagger",    2 , 30},
...
const struct lookup_sect tab_sector_types[] =
{
  { "nada",			0,0, "!" },
  { "city",                     1, 50, "{W+{x"},
  { "field",                    2, 50, "{G.{x"},
  ...
  { "tundra",                  18, 50, "{wT"},
  { "wasteland",               19, 50, "{WW"},
  { "dense_forest",            20, 50, "{GF"},
  { "lava",                    21, 50, "{R="},
  { "rock_mountain",           22, 50, "{d^"},
  { "snow_mountain",           23, 50, "{W^"},
  { "bog",                     24, 50, "{GB"},
  { "ruins",                   25, 50, "{R#"},
  { "underground",             26, 50, "{w#"},
  { "house",                   27, 50, "{rH"},
...
const struct lookup_type tab_sector_typess[] =
{
  { "nada",			0,0},
  { "city",                     1, 50},
  { "field",                    2, 50,},
...
char * show_values(const struct lookup_type * table,int value, bool fBit)
{
     ...
     for (a=0;table[a].text!=NULL;a++)
     {
         ...
         && ( strcmp(table[a].text,"placeholder") )  )
      {
       strcat(buf,"      ");
       sprintf( tmp, "%s%-13s", 
	 fBit?  (IS_SET( value, table[a].value ) ? "{Y*" : "{w ") :
		(value == table[a].value ? "{Y*" : "{w "),
       table[a].text );
       strcat(buf,tmp );
       if ( ++foo % 4 == 0 )
	  strcat(buf,"\n\r");
...
char * show_valuess(const struct lookup_sect *table, int value, bool fBit)
{

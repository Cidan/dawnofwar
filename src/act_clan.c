...
char *get_diplo_name( sh_int value )
{
  ...
  if ( value < -450 )			name = "{r  WAR  {x";
  else if ( value < -300 )		name = "{R HATRED{x";
  else if ( value < -150 )		name = "{dDISLIKE{x";
  else if ( value < 150 )		name = "{WNEUTRAL{x";
  else if ( value < 300 )		name = "{CRESPECT{x";
  else if ( value < 450 )		name = "{B TRUST {x";
  else					name = "{b ALLY  {x";
...
void do_politics( CHAR_DATA *ch, char *argument )
{
  ...
  send_to_char( "{xCurrent Politics of " mudnamecolor "\n\r\n\r", ch );
  buf[0] = '\0';
  buf2[0] = '\0';
  ...
  for ( x =1 ; x <MAX_CLAN ; x++ )
  {
    sprintf( buf, " %s  ", get_clan_info(x,CABBR) );
    safe_strcat(  MAX_STRING_LENGTH, buf2, buf );
  }
  buf[0] = '\0';
  sprintf( buf, "\n\r\n\r" );
  ...
  for ( x=1; x < MAX_CLAN ; x++ )
  {
    ...
    buf[0] = '\0';
    buf2[0] = '\0';
    sprintf( buf,"%1i %s ", x, get_clan_info(x,CABBR) );
    safe_strcat(  MAX_STRING_LENGTH, buf2, buf );
  ...
  if ( IS_SET( ch->pcdata->pflags, PFLAG_CLAN_DIPLOMAT ) )
    for ( x = 1; x < MAX_CLAN; x++ )
    {
      ...
      {
        sprintf( buf, "%s has requested an end to your current state of affairs", get_clan_info(x,CNAME) );
        send_to_char( buf, ch );
      }
    }
...
void do_negotiate( CHAR_DATA *ch, char *argument )
{
   ...
   if ( politics_data.daily_negotiate_table[ch->pcdata->clan] [ target_clan ] )
   {
     sprintf( buf, "Your clan has already negotiated with %s today.\n\r", 
      get_clan_info(target_clan,CNAME) );
     send_to_char( buf, ch );
     return;
   }
   ...
   if ( !str_prefix( arg2, "peace" ) )
   {
     if  ( politics_data.diplomacy[ch->pcdata->clan][target_clan] < -450 )
     {
                ...
                politics_data.end_current_state[target_clan][ch->pcdata->clan] )
       {
         ...
         send_to_char( "{xYou have successfully negotiated an end to this dreaded {RCLAN WAR{x. Great Job!!\n\r", ch );
         sprintf( buf, "{RCLAN:{x The war between %s and %s has ended. They may no longer PKILL each other!!\n\r",
            get_clan_info(ch->pcdata->clan,CNAME), get_clan_info(target_clan,CNAME) );
         info( buf, 1);
       ...
       {
         sprintf( buf, "{xYou are currently at {rWAR{x with %s. Both clans must negotiate an end to the war first.\n\r", 
                       get_clan_info(target_clan,CNAME));
         send_to_char( buf, ch );
         return;
       }
   ...
   else if ( !str_prefix( arg2, "war" ) )
   {
     ...
     {
       sprintf( buf, "{xYou have started a {RWAR{x with %s! Watch out!.\n\r", 
                     get_clan_info(target_clan,CNAME));
       send_to_char( buf, ch );

       sprintf( buf, "{RCLAN:{x A war has started between %s and %s. They may now PKILL each other!!\n\r",
          get_clan_info(ch->pcdata->clan,CNAME), get_clan_info(target_clan,CNAME) );
       info( buf, 1); 
...
void do_cset( CHAR_DATA *ch, char *argument )
{
	...
	if(!str_cmp(get_clan_info(value, CNAME), "None") )
	{
    ...
    victim->pcdata->clan = value;
	     for (clan = first_clan; clan != NULL; clan = clan->next)
		   ...
		   break;
	  sprintf(mem, "%s ", clan->members);
	  strcat(mem, victim->name);
      clan->members = str_dup(mem);

    sprintf( buf, "%s now belongs to clan %s.\n\r", victim->name,
        get_clan_info(value,CNAME) );
    send_to_char( buf, ch );
...
void do_accept( CHAR_DATA *ch, char *argument )
{
   ...
   victim->pcdata->clan = ch->pcdata->clan;
	  ...
	  sprintf(mem, "%s ", clan->members);
	  strcat(mem, victim->name);
	  free_string(clan->members);
      clan->members = str_dup(mem);

   act( "$N accepts you into $S clan!", victim, NULL, ch, TO_CHAR );
   act( "You accept $N into your clan!", ch, NULL, victim, TO_CHAR );
   
   sprintf( buf, "%s has accepted %s into clan %s.", ch->name, victim->name,
      get_clan_info(ch->pcdata->clan,CNAME) );
   monitor_chan( buf, MONITOR_CLAN );
   save_clans();
...
/* Bleh, small hack for clan member list.
 * Would spew out extra garbage otherwise.
 */
void fix_hack(CLAN_DATA *clan)
{
	char zoop[MSL];
	int num = 0;
	int len;
	len = strlen(clan->members);
	while( num < len)
	{
		if(isspace(clan->members[num])
			&& !IS_LETTER(clan->members[num + 1]) )
		{
			zoop[0] = '\0';
			strcat(zoop, clan->members);
			free_string(clan->members);
			zoop[num] = '\0';
			clan->members = str_dup(zoop);
			zoop[0] = '\0';
		}
	num++;
	}
...
/* Fix for clan->enemies list */
void fix_enemies(CLAN_DATA *clan)
{
	char zoop[MSL];
	int num = 0;
	int len;
	len = strlen(clan->enemies);
	while( num < len)
	{
		if(isspace(clan->enemies[num]) && !IS_NUMBER(clan->enemies[num + 1])  )
		{
			zoop[0] = '\0';
			strcat(zoop, clan->enemies);
			free_string(clan->enemies);
			zoop[num] = '\0';
			clan->enemies = str_dup(zoop);
			zoop[0] = '\0';
		}
	num++;
	}
...
void do_leave( CHAR_DATA *ch, char *argument )
{
   ...
   if ( IS_SET( ch->pcdata->pflags, PFLAG_CLAN_BOSS ) )
   {
	   send_to_char("Abandon YOUR clan? When you can kick anyone out?\n\rAsk an Imm to help you.\n\r", ch);
	   return;
   }
   modbuf[0] = '\0';
   sprintf( buf, "%s has left clan %s.", 
      ch->name, get_clan_info(ch->pcdata->clan,CNAME) );
   monitor_chan( buf, MONITOR_CLAN );
...
void do_banish( CHAR_DATA *ch, char *argument )
{
   ...
   if ( victim->pcdata->clan != ch->pcdata->clan || victim->pcdata->clan == 0 )
   {
   ...
   sprintf( buf, "%s has banished %s from clan %s.", ch->name, 
      victim->name, get_clan_info(ch->pcdata->clan,CNAME) );
   monitor_chan( buf, MONITOR_CLAN );
   
   sprintf( buf, "$n banishes you from clan %s!",
	   get_clan_info(ch->pcdata->clan,CNAME) );
...
void do_make( CHAR_DATA *ch, char *argument )
{
   ...
   if ( !is_number( arg1 ) || arg2[0] == '\0' )
   {
      ...
      {
          if ( get_clan_eq(ch->pcdata->clan, cnt) != -1 
            && ( ( pObj = get_obj_index( get_clan_eq(ch->pcdata->clan, cnt) ) )!= NULL ) )
          {
   ...
   if ( get_clan_eq(ch->pcdata->clan, num) == -1 )
   {
   ...
   if ( ( pObj = get_obj_index(get_clan_eq(ch->pcdata->clan, num) ) ) == NULL )
   {
...
void do_council( CHAR_DATA *ch, char *argument )
{
  ...
  if (  ( IS_NPC( ch )  )
     || ( !IS_SET( ch->pcdata->pflags, PFLAG_SUPER_COUNCIL )  )  ) 
  {
  ...
  if ( !str_prefix( arg1, "accept" )  )
  {
    ...
    else
    {
      if ( ( victim = get_char_room( ch, arg2 ) ) != NULL ) 
      {
        ...
        if ( this_council == SUPER_VAMP && !IS_VAMP( victim ) )
        {
        ...
        {
          sprintf( buf, "%s is now a member of the Council of %s!\n\r", victim->name, super_councils[this_council].council_name );
          send_to_char( buf, ch );
  ...
  if ( !str_prefix( arg1, "outcast" )  )
  {
    ...
    else
    {
      if ( ( victim = get_char_world( ch, arg2 ) ) != NULL ) 
      {
        ...
        {
          sprintf( buf, "%s has been outcast from the {RKindred{x by the %s, and is now a {dRENEGADE{x!!!!\n\r",
           victim->name, super_councils[this_council].council_name );
          send_to_char( buf, ch );
          sprintf( buf, "You have been {rOUTCAST{x from the {RKindred{x by the %s, and are now a {dRenegade{x!!!!!\n\r",

...
#define IS_LOWER(c)             ((c) >= 'a' && (c) <= 'z' ? TRUE : FALSE)
#define UPPER(c)                ((c) >= 'a' && (c) <= 'z' ? (c)+'A'-'a' : (c))
#define IS_UPPER(c)             ((c) >= 'A' && (c) <= 'Z' ? TRUE : FALSE)
#define IS_SET(flag, bit)       ((flag) & (bit))
/*
void SET_BIT(long *var, long bit) {
	var |= (1 << bit)
}
*/
...
#define GET_AC(ch)              ( IS_NPC(ch) ? (REAL_AC( ch ) + ch->ac_mod) : REAL_AC( ch ))
/*
#define GET_AC(ch,type)		((ch)->armor[type] + ( IS_AWAKE(ch) ? dex_app[get_curr_stat(ch,STAT_DEX)].defensive : 0 ))
*/
#define REAL_AC(ch)             ((ch)->armor + ( IS_AWAKE(ch) ? dex_app[get_curr_dex(ch)].defensive : 0 ))
...
/*
#define GET_HITROLL(ch) ( (ch)->hitroll+str_app[get_curr_stat(ch,STAT_STR)].tohit+dex_app[get_curr_stat(ch,STAT_DEX)].tohit )
#define GET_DAMROLL(ch) ((ch)->damroll+str_app[get_curr_stat(ch,STAT_STR)].todam)
*/
#define GET_HITROLL(ch)         ( IS_NPC(ch) ? (REAL_HITROLL(ch) + ch->hr_mod + (get_psuedo_level( ch ) / 4 )) : REAL_HITROLL(ch)+(ch->level/8) )

#define REAL_HITROLL(ch)        ( (ch)->hitroll+str_app[get_curr_str(ch)].tohit+dex_app[get_curr_dex(ch)].tohit )

#define GET_DAMROLL(ch)         ( IS_NPC(ch) ? (REAL_DAMROLL(ch) + ch->dr_mod + (ch->level / 3 ))  : REAL_DAMROLL(ch)+(ch->level/10) )

#define REAL_DAMROLL(ch)        ((ch)->damroll+str_app[get_curr_str(ch)].todam )
 ...
 * Description macros.
 */
#define PERS(ch, looker) ( can_see( looker, (ch) ) ? ( IS_NPC(ch) ? (ch)->short_descr : (IS_WOLF(ch) && (IS_SHIFTED(ch) || IS_RAGED(ch))) ? "A Large {yWolf{x" :(ch)->name ) : IS_IMMORTAL( ch ) ? "A Mystical Being" : "Someone" )

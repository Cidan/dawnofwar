...
void advance_level( CHAR_DATA *ch, int class, bool show, bool remort )
{
    ...
    if ( class == ADVANCE_WOLF )
    {
      ...
      sprintf( buf,
	"{xYou gain: %d {GRage Ability{x, and %d {yWerewolf Practices. .{x\n\r",
    ...
    add_prac =number_range( 1, UMAX( 2, ( ( MAX_VAMP_LEVEL / 2 ) - (ch->pcdata->generation ) ) 
      ...
      sprintf( buf,
	"You gain: %d {RBloodlust{x, and %d Vampyre Practices. .\n\r",
	add_bloodlust, add_prac );
...
//		     class_table[class].hp_max );
	...
	if (number_range(1, 20000) < get_curr_lck(ch))
	{
	ch->train           += 3;
	tluck = TRUE;
	}
	...
	if (!tluck)
    sprintf( buf,
	"You gain: %d Hit Points, %d Mana, %d Movement, %d pracs and 1 train.\n\r",
	add_hp, add_mana, add_move, add_prac );
	else
    sprintf( buf,
	"You gain: %d Hit Points, %d Mana, %d Movement, %d pracs.\n\r{YYou LUCKED OUT! You gained 3 trains!{x\n\r",
	add_hp, add_mana, add_move, add_prac );
...
void hunt_update( void )
{
		...
		if (ch->searching != NULL)
	{
		if ((quitter = get_char_world(ch, ch->searching)) != NULL)
			set_hunt(ch, NULL, quitter, NULL, HUNT_WORLD | HUNT_OPENDOOR | HUNT_PICKDOOR|HUNT_UNLOCKDOOR|HUNT_INFORM,HUNT_CR|HUNT_MERC);
...
/* Moved to Hunt Update which is called alot more often.
...
void clean_donate_rooms( void )
{
  ...
  {
    for ( room_count = 0, object = room->first_content;
          object;
    ...
    if ( room_count > 150 )
    {
      ...
      for ( object = room->first_content; object; object = obj_next )
      {
        ...
        if (  ( IS_SET( object->extra_flags, ITEM_RARE ) )
           || ( object == quest_object ) )
          chance = 0;
  ...
  {
    if ( ( room_vnum = get_clan_numin(looper, CDONA) ) == 0 )
      continue;
...
void weather_update( void )
{
    ...
    buf[0] = '\0';
    buf2[0] = '\0';
	mud_time = localtime(&current_time);

	time_info.min = mud_time->tm_min;
	time_info.hour = mud_time->tm_hour;
	time_info.day = mud_time->tm_mday;
	time_info.month = mud_time->tm_mon;
	time_info.year = mud_time->tm_year;
	time_info.moon = time_info.hour;
...
//	time_info.day++;
    ...
    case 18 :
        ...
        sprintf( buf2, "{xA %s {YMoon {xhas risen.\n\r", get_moon_phase_name( ) );
        safe_strcat( MSL, buf, buf2);
        break;
    case 20 :
        weather_info.moon_loc = MOON_LOW;
        sprintf( buf2, "{xThe %s {YMoon {xrides low on the horizon.\n\r", get_moon_phase_name( ) );
        safe_strcat( MSL, buf, buf2);
        break;
    case 0 :
        weather_info.moon_loc = MOON_PEAK;
        sprintf( buf2, "{xThe %s {YMoon {xreaches it's zenith.\n\r", get_moon_phase_name( ) );
        safe_strcat( MSL, buf, buf2);
        break;
     case 3 :
        weather_info.moon_loc = MOON_FALL;
        sprintf( buf2, "{xThe %s {YMoon {xfalls.\n\r", get_moon_phase_name( ) );
        safe_strcat( MSL, buf, buf2);
        break;
     case 6 :
        weather_info.moon_loc = MOON_SET;
        sprintf( buf2, "{xThe %s {YMoon {xis setting.\n\r", get_moon_phase_name( ) );
        safe_strcat( MSL, buf, buf2);
        break;
     case 8 :
        weather_info.moon_loc = MOON_DOWN;
        sprintf( buf2, "{xThe %s {YMoon {xhas left the sky.\n\r", get_moon_phase_name( ) );
        safe_strcat( MSL, buf, buf2);
        break;
...
/* Update super_councils info  */
   ...
   for ( ch = first_char; ch != NULL; ch = ch->next )
   {
	 ...
	 if (  ( ch->move < ch->max_move )
         ...
         {
           send_to_char( "You are carrying so much wieght that you are {REXHAUSTED{x!!\n\r", ch );
           ch->move = 0;
         }
  ...
  /*    if ( ch->stunTimer > 0 )
	...
	if ( ( !IS_NPC(ch) && ch->level < LEVEL_IMMORTAL ) )
	{
        ...
        if ( obj->value[7] > 0 )
        {
          ...
          {
            sprintf( bug_buf, "ERROR in expiring item %s(%s %d): item has a replace vnum set (%d), but that is not a valid item.",
		...
		paf->duration--;
              ...
              if ( paf->type == skill_lookup( "black hand" ) )
	        {
                 ...
                 && ( ch->hit > 10 )   )
              {
                ...
                send_to_char( "{xYou feel the affects of your {Radrenaline rush{x wear off, leaving you exhausted.\n\r", ch );
...
{   
	...
	char *message;
     ...
     if ( IS_SET( obj->item_apply, ITEM_APPLY_HEATED ) 
	  ...
	  if ( obj->carried_by != NULL )
	  {
	    act( "Your $p {Ccools off{x!!", obj->carried_by, obj, NULL, TO_CHAR );
	  }
	  else 
        if ( obj->in_room != NULL
	       &&      ( rch = obj->in_room->first_person ) != NULL )
	  {
	    act( "$p {Ccools off{x!!", rch, obj, NULL, TO_ROOM );
	    act( "$p {Ccools off{x!!", rch, obj, NULL, TO_CHAR );
	  }
...
void auction_update( void )
 {
    ...
    switch ( auction_stage )
    {
	  ...
	  {
	     sprintf( buf, "{x%s (level:%d, valued at %s) has been offered for auction.  A {R10%% fee{x will be charged, the higher of the reserve price or highest bid.",

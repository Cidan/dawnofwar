...
char * show_valuess args ( (const struct lookup_sect *table, int value, bool fBit) );
...
const   struct  cmd_type        build_cmd_table   [] =
{
	...
	{ "autobuild",    build_autodig,  POS_STANDING,   0,  LOG_NORMAL },
    /*
     * End of list.
     */
...
ROOM_INDEX_DATA *  new_room( AREA_DATA * pArea, sh_int vnum, sh_int sector )
{
  ...
  {
    char testbuf[MSL];
    sprintf( testbuf, "loading rooms, vnum %d", 
      pRoomIndex->vnum );
...
/*    int trust; Unused var*/
	...
	argument++;
/* Actually... instead of returning on a NULL argument here
 * We want it to show the room/obj/mob just like if you typed 'x'
 */
    if ( argument[0] == '\0' )
	{
		build_list(ch, "");
...
/*    RESET_DATA     *pReset; Unused var */
    ...
    sprintf( buf, "{WName:{x %s.    {WClass:{x %s.\n\r",
	pMob->player_name, "Fix" );
    strcat( buf1, buf );

    sprintf( buf, "{WVnum:{x %d.  {WSex:{x %s.  {WRace:{x %s ",
	pMob->vnum,
	pMob->sex == SEX_MALE    ? "male"   :
	pMob->sex == SEX_FEMALE  ? "female" : "neutral",
  (  ( pMob->race > -1 ) && ( pMob->race < MAX_RACE ) ) ?
  race_table[ pMob->race].race_title : "Illegal Race"
	);
    strcat( buf1, buf );
    ...
    strcat( buf1, buf );
    sprintf(buf, "{WHP:{x %d   {WMP:{x %d{x   {WHometown:{x  %s{x\n\r",
        pMob->hp, pMob->mp, (pMob->hometown == 0 ? "None" : hometown_table[pMob->hometown].name) );
	strcat(buf1, buf);

    sprintf( buf, "{WModifiers: AC:{x %d.  {WHitroll:{x %d.  {WDamroll:{x %d.\n\r",
		 pMob->ac_mod,
		 pMob->hr_mod,
		 pMob->dr_mod );
    strcat( buf1, buf );
	...
	buf1[0] = '\0';
	buf[0] = '\0';
    sprintf( buf, "{WMob Flags:{x\n\r%s",
       show_values( tab_mob_flags, pMob->act, TRUE ) );
    strcat( buf1, buf );
    
    sprintf( buf, "{WAffected by:{x\n\r%s",
       show_values( tab_affected_by, pMob->affected_by, TRUE ) );
    strcat( buf1, buf );
    sprintf( buf, "{WChance for item:{x %d\n\r",
        pMob->chance_of_item);
    strcat( buf1, buf );
    
    sprintf( buf, "{WSkill Flags:{x %s\n\r",
       bit_table_lookup( tab_mob_skill, pMob->skills ) );
    strcat( buf1, buf );
    sprintf( buf, "{WPSkill Flags:{x %s\n\r",
       bit_table_lookup( tab_mob_powerskill, pMob->power_skills ) );
    strcat( buf1, buf );    
    sprintf( buf, "{WCast Flags:{x %s\n\r",
       bit_table_lookup( tab_mob_cast, pMob->cast ) );
    strcat( buf1, buf );
    sprintf( buf, "{WPCast Flags:{x %s\n\r",
       bit_table_lookup( tab_mob_powercast, pMob->power_cast ) );
    strcat( buf1, buf );    
    sprintf( buf, "{WDef Flags:{x %s\n\r",
       bit_table_lookup( tab_mob_def, pMob->def ) );
    strcat( buf1, buf );
    sprintf( buf, "{WStrong Magic Realms:{x %s\n\r",
       bit_table_lookup( tab_magic_realms, pMob->strong_magic ) );
    strcat( buf1, buf );
    sprintf( buf, "{WWeak Magic Realms:{x %s\n\r",
       bit_table_lookup( tab_magic_realms, pMob->weak_magic ) );
    strcat( buf1, buf );
    sprintf( buf, "{WResist Magic Realms:{x %s\n\r",
       bit_table_lookup( tab_magic_realms, pMob->resist ) );
    strcat( buf1, buf );
    sprintf( buf, "{WSuscept Magic Realms:{x %s\n\r",
       bit_table_lookup( tab_magic_realms, pMob->suscept ) );
    strcat( buf1, buf );
    sprintf( buf, "{WRace Mods:{x %s\n\r",
       bit_table_lookup( tab_mob_race_mods, pMob->race_mods ) );
    strcat( buf1, buf );    

    sprintf( buf, "{WShort description:{x %s.\n\r{WLong description:{x %s\n\r",
	pMob->short_descr,
    ...
    if ( pMob->spec_fun != 0 )
      {
	sprintf(buf,"{WMobile has spec fun:{x %s\n\r",rev_spec_lookup(pMob->spec_fun) );
	strcat( buf1, buf);
      }
    if ( pMob->spec_fun == spec_thant_scout)
    {
        strcat(buf1, "{WMobile is a scout{x\n\r");
        sprintf(buf, "{WRoom to report to:{x %d\n\r", pMob->report_room);
        strcat(buf1, buf);
    }
    ...
    if ( (pShop=pMob->pShop) != 0)
      {
	is_shopkeeper=1;
	strcat( buf1, "{WMobile is a shopkeeper, will buy{x ");
	...
	strcat( buf1, "\n\r");
	sprintf(buf,"{WOpens at {R%i {Whrs, shuts at {R%i {Whours, profbuy:{R%i{W, {Wprofsell:{R%i{W.\n\r",
...
void build_showobj( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "{WName:{x %s  {WLevel:{x %d.\n\r",      obj->name, obj->level );
    strcat( buf1, buf );

    sprintf( buf, "{WVnum:{x %d.  {WType:{x %s.\n\r",
 ...
 /*     bit_table_lookup(tab_extra_flags, obj->extra_flags ) );
    ...
    sprintf( buf, "{WItem_applies:{x %s.\n\r", 
             bit_table_lookup( tab_item_apply, obj->item_apply ) );
    strcat( buf1, buf );

    sprintf( buf, "{WWeight:{x %d.\n\r", obj->weight );
    strcat( buf1, buf );
    ...
    for ( cnt = 0; cnt < 10; cnt++ )
    {
       ...
       else if ( is_name( "Hit", rev_table_lookup( tab_value_meanings, ( obj->item_type * 10 ) + cnt ) ) )
       {
       ...
       {
          foo = rev_table_lookup( tab_attack_types, obj->value[cnt] );
          if ( foo[0] == '\0' )
             sprintf( buf, "                  {r(INVALID!){w\n\r" );
          else      
             sprintf( buf, "                  {Y(%s){w\n\r", foo );
    ...
    if ( obj->obj_fun != NULL )
    {
       sprintf( buf, "{WObject has objfun:{x %s.{x\n\r", 
	...
	strcat( buf1, "{WExtra description keywords: '{x" );
    ...
    {
	sprintf( buf, "{WAffects{x {g%s{x {Wby{x {g%d{W.{x\n\r",
...
void build_showroom( CHAR_DATA *ch, char *argument )
{
	...
	"{WVnum:{x %d\n\r{WSector:{x\n\r%s",
	location->vnum,
	show_valuess(tab_sector_types, location->sector_type , FALSE ) );
    strcat( buf1, buf );

    send_to_char(buf1, ch);
	buf1[0] = '\0';

    sprintf( buf, "{WFlags:{x\n\r%s",
       show_values( tab_room_flags, location->room_flags, TRUE ) );
    strcat( buf1, buf );
    
    if ( (display & DISPLAY_DESC) )
    {
     sprintf( buf, "{WDescr:{x \n\r%s\n\r\n\r", location->description);
     strcat( buf1,buf);
    }
	...
	if ( (display & DISPLAY_FULLDOORS) )
	 {
	  if (pexit->key>0)
	       ...
	       pKeyObj=NULL;
	  sprintf( buf,"{WExit:{x {y%7s {W: To {y%5i %s\n\r",sDirs[door],
	  ...
	  sprintf( buf,"             {WKey: {y%5i %s,  {WExit Type:{y %s\n\r",
	   ...
	   pKeyObj != NULL ? pKeyObj->name : "None",
	  ...
	  sprintf(buf,"{WDesc: {y%s",pexit->description[0] != '\0'
   ...
   if ( (display & DISPLAY_RESETS ) &&
	 location->first_room_reset != NULL)
   {
     strcat (buf1, "{WResets:\n\r");
...
/*    char *argn, *oldperm; unused */
  ...
  if (!is_number(arg1))
  {
   ...
   if ( !str_cmp( arg2, "sex" ) )
   {
	...
	{
	    send_to_char( "HP range is 1 to 30,000.\n\r", ch );
	    return;
	}
	pMob->hp = value;
    send_to_char( "HP set.\n\r", ch );
	...
	if ( value < 0 || value > 4 )
	{
	    send_to_char( "Valid range is 0 to 4, 0 for none.\n\r", ch );
	    return;
	}
	pMob->hometown = value;
	...
	if ( value < 1 || value > 30000 )
	{
	    send_to_char( "MP range is 1 to 30,000.\n\r", ch );
	    return;
	}
	pMob->mp = value;
	send_to_char( "MP set.\n\r", ch );
    ...
    if ( !str_cmp( arg2, "report") )
    {
        ...
        {
            send_to_char( "Room to report to must be a VALID VNUM.\n\r", ch);
            return;
        }
        pMob->report_room = value;
        send_to_char( "Report to vnum set.\n\r", ch);
        area_modified(pArea);
        return;
    }

	if ( !str_cmp( arg2, "random_chance" ) )
    {
	if ( value < 0 || value > 100 )
	{
	    send_to_char( "Random percent of item appearing range is 0 to 100.\n\r", ch );
	    return;
	}
	pMob->chance_of_item = value;
    ...
    if ( !str_cmp( arg2, "spec" ) )
    {
        ...
        if ( arg3[0]=='-' )
        {
         ...
         if (pList != NULL)
         {
          UNLINK(pList, pArea->first_area_specfunc,
                 pArea->last_area_specfunc, next, prev);
	...
	if (pList == NULL)
	{
	    ...
	    pList->data = pMob;
	    LINK(pList, pArea->first_area_specfunc,
	         pArea->last_area_specfunc, next, prev);
	}
    ...
    if ( !str_cmp( arg2, "shop" ) )
    {
     ...
     if ( !str_cmp(arg3, "clear") )
	  ...
	  if (pShop!=NULL)
	  {
	   ...
	   if (pList != NULL)
	   {
	    UNLINK(pList, pArea->first_area_shop, pArea->last_area_shop,
	           next, prev);
	    PUT_FREE(pList, build_free);
	   }
     ...
     if ( (str_prefix("trade",arg3)
	    && !is_name(arg3,"profbuy profsell openhour closehour") ) )
      {
	  /* Usage message */
	  build_setmob( ch,"" );
	  return;
     ...
     if (pShop==NULL)
     {
      ...
      LINK(pList, pArea->first_area_shop, pArea->last_area_shop,
           next, prev);
...
void nuke_exit_resets(ROOM_INDEX_DATA *pRoomIndex, int door)
{
  ...
  for (pList=pRoomIndex->first_room_reset; pList; pList=pListNext)
  {
    ...
    {
      /* nuke this reset */
      UNLINK(pList, pRoomIndex->first_room_reset, pRoomIndex->last_room_reset,
	     next, prev);
      PUT_FREE(pList, build_free);
...
void build_setroom( CHAR_DATA *ch, char * argument)
{
	...
	send_to_char( "                         seperate <-- {RUSE THIS IF YOU SET A KEYWORD{x\n\r",     ch );
   ...
   if ( value != 0 )
   {
      ...
      sprintf( buf, "{WRoom flag {Y%s{W toggled to {Y%s{W.{w\n\r",
         rev_table_lookup( tab_room_flags, value ),
         IS_SET( location->room_flags, value ) ? "ON" : "OFF" );
      send_to_char( buf, ch );
      return;
   }

   value = table_lookup( tab_sector_typess, arg1 );
   if ( value >= 1 )
   {
     /*  Sector found */
      location->sector_type = value;
      sprintf( buf, "{WSector type set to {Y%s{W.{w\n\r",
         rev_table_lookup( tab_sector_typess, value ) );
      send_to_char( buf, ch );
      return;
   }

    if ( !str_cmp( arg1, "edit" )
    && is_name( arg2, "name desc" ) )
    {
	...
	if (argn[0]=='-')
	 {
	  ...
	  /* Delete description */
	  UNLINK(ed, location->first_exdesc, location->last_exdesc,
	         next, prev);
	...
	if (! is_name(arg3,"to key desc keyword type seperate"))
	{
	 build_setroom(ch,"");
	 return;
	}
	...
	if (! str_cmp(arg3,"to"))
	{
	 ...
	 if (   pDestRoom != NULL
	     && !build_canwrite(pDestRoom->area,ch,0) )
	    {
	 ...
	 if (str_cmp(arg5,"onesided")) /* If NOT onesided, do checks */
	 {
	  ...
	  if ( pDestExit->to_room != location )
	  {
	   sprintf(buf,"The door in room %i going %s points to room [%i] %s.\n\r",
			...
			sDirs[RevDirs[door]],
			pDestExit->vnum,
			pDestExit->to_room != NULL ? pDestExit->to_room->name : ""
			);
	   send_to_char(buf,ch);
	  }
	 }
	...
	if (! str_cmp(arg3,"keyword") )
	{
     ...
     build_strdup(&pDestExit->keyword,arg4,TRUE,ch);
     if (arg4[0] == '\0')
     {
     pExit->is_named = FALSE;
     pDestExit->is_named = FALSE;
     }
     else
     {
     pExit->is_named = TRUE;
     pDestExit->is_named = TRUE;
     }
	 return;
	}
     if (! str_cmp(arg3,"seperate") )
	{
     if (pExit->seperate_plane == FALSE)
     {
         pExit->seperate_plane = TRUE;
         pDestExit->seperate_plane = TRUE;
         send_to_char("Seperate plane is {RON{x. {RMAKE SURE THERE ARE NO OTHER {Y'NON-KEYWORD'{R EXITS TO A DIFFRENT PLANE.{x\n\r", ch);
     }
     else
     {
         pExit->seperate_plane = FALSE;
         pDestExit->seperate_plane = FALSE;
         send_to_char("Seperate plane is {GOFF{x. {RMAKE SURE THERE ARE NO OTHER {Y'KEYWORD'{R EXITS TO A DIFFRENT PLANE.{x\n\r", ch);
     }
	 return;
	}
...
void build_setobject( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( arg2, "objfun" ) )
    {
        ...
        if ( arg3[0]=='-' )
        {
         ...
         if (pList != NULL)
         {
          UNLINK(pList, pArea->first_area_objfunc, pArea->last_area_objfunc,
                 next, prev);
          PUT_FREE(pList, build_free);
         }
	...
	if (pList == NULL)
	{
	    GET_FREE(pList, build_free);
	    pList->data = pObj;
	    LINK(pList, pArea->first_area_objfunc, pArea->last_area_objfunc,
	         next, prev);
	}
...
void build_dig( CHAR_DATA *ch, char *argument )
{
    ...
    LINK(pList, pCurRoom->area->first_area_room,
         pCurRoom->area->last_area_room, next, prev);
    top_room++;
...
void do_build( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char("Autobuild is OFF.{x\n\r", ch);
	ch->pcdata->autobuild = FALSE;
    if ( ch->act_build == ACT_BUILD_REDIT && ch->build_vnum != ch->in_room->vnum )
    {   
       ch->build_vnum = ch->in_room->vnum;
...
void build_addreset( CHAR_DATA *ch, char *argument )
{
    ...
    if (! str_cmp(arg1,"door"))
    {
	 ...
	 return;
       ...
       if ((pExit=pRoomIndex->exit[rarg2])==NULL ||
    ...
    else
    {
     ...
     LINK(pList, pRoomIndex->first_room_reset, pRoomIndex->last_room_reset,
          next, prev);
    }
    area_modified( pArea );
    return;
...
void build_delreset( CHAR_DATA *ch, char *argument )
{
  ...
  if (!pReset)
  {
    bugf("delreset: pList returned null in search for reset number %d",
         atoi(arg1));
  ...
  if (pReset->command=='M')
  {
    ...
    UNLINK(pList, pRoomIndex->first_room_reset,
...
void build_delroom( CHAR_DATA *ch, char *argument )
{
    ...
    /* Delete all exits from this room too */
    {
     ...
     for (pList=pArea->first_area_room;pList != NULL;pList=pList->next)
     {
      ...
      for (door=0;door<=5;door++)
      {
       if ( (pExit=pSrchRoom->exit[door]) != NULL
	    && (pExit->to_room == pRoomIndex ||
	    pSrchRoom == pRoomIndex) )
       {
	/* Get rid of exit. */
	PUT_FREE(pExit, exit_free);
    ...
    /* wipe off resets referencing room. */
    {
	 ...
	 if (pReset->arg3 == vnum) found=1;
	 break;
       case 'O':
	if (pReset->arg3 == vnum) found=1; break;
       case 'D':
       case 'R':
	if (pReset->arg1 == vnum) found=1; break;
       case 'G':
       case 'E':
	if (iLastMobRoom==vnum) found=1; break;
      }
...
void build_delobject( CHAR_DATA *ch, char *argument )
{
	...
	if (pReset->arg1 == vnum) found=1; break;
       case 'P':
	if (pReset->arg1 == vnum || pReset->arg3 == vnum) found=1; break;
       case 'G':
       case 'E':
	if (pReset->arg1==vnum) found=1; break;
      }
...
void build_delmob( CHAR_DATA *ch, char *argument )
{
    ...
    if ( pList )
    {
      UNLINK(pList, pArea->first_area_mobile, pArea->last_area_mobile,
             next, prev);
      PUT_FREE(pList, build_free);
    }
	...
	if (pReset->arg1 == vnum) found=1; break;
       case 'G':
       case 'E':
	if (iLastMob==vnum) found=1; break;
      }
...
void build_set_oedit( CHAR_DATA *ch, char *argument )
{
   ...
   send_to_char( ch->build_vnum == -1 ? "Invalid vnum. Oedit <vnum>.\n\r" 
		 : "Vnum now set. " , ch );
   return;
}
...
void build_set_redit( CHAR_DATA *ch, char *argument )
{
   ...
   send_to_char( "Autobuild is set to {ROFF{x as a precaution.\n\r", ch );
   return;
}
...
void build_set_medit( CHAR_DATA *ch, char *argument )
{
   ...
   send_to_char( ch->build_vnum == -1 ? "Invalid Vnum. Medit <vnum>\n\r" 
		 : "Vnum now set. " , ch );
   return;
}
...
void build_list( CHAR_DATA *ch, char *argument )
{
   ...
   /* Ok, now arg is valid.  See if it applic. to edit mode */
	 ...
	 wide_table_printout(tab_sector_typess,buf+strlen(buf));
	 send_to_char(buf,ch);
      }
...
void build_listvalues( CHAR_DATA *ch, char *argument )
{
   ...
   for ( foo = 0; foo < 4; foo++ )
   {
      sprintf( buf, "{W[Value{Y%d{W] : {Y%s{w\n\r",
...
void build_listweapons( CHAR_DATA *ch, char *argument )
{
   ...
   {
      sprintf( buf, "{W%2ld - {Y%10s.   ", tab_weapon_types[foo].value,
				  tab_weapon_types[foo].text );
      send_to_char( buf, ch );
      if ( (foo+1) % 2 == 0 )
	 send_to_char( "\n\r", ch );
   }
...
void build_listliquids( CHAR_DATA *ch, char *argument )
{
   ...
   {
      sprintf( buf, "%2ld - %12s.   ", tab_drink_types[foo].value,
				  tab_drink_types[foo].text );
      send_to_char( buf, ch );
      if ( (foo+1) % 2 == 0 )
	 send_to_char( "\n\r", ch );
   }
...
void build_listspells( CHAR_DATA *ch, char *argument )
{
   ...
   buf2[0] = '\0';
	...
	if ( !strcmp( argument, "all" ) ) fall = TRUE;
   else if ( !strcmp( argument, "ign" ) ) type = TAR_IGNORE;
   else if ( !strcmp( argument, "off" ) ) type = TAR_CHAR_OFFENSIVE;
   else if ( !strcmp( argument, "def" ) ) type = TAR_CHAR_DEFENSIVE;
   else if ( !strcmp( argument, "slf" ) ) type = TAR_CHAR_SELF;
   else if ( !strcmp( argument, "obj" ) ) type = TAR_OBJ_INV;
   else
   {
   ...
   for ( sn = 0; sn < MAX_SKILL; sn++ )
   {
	 ...
	 continue;
      ...
      {
	 sprintf( buf, "{W[Spell No: %4d] {Y%s{w\n\r", sn,
		  skill_table[sn].name );
	 strcat( buf2, buf );
      }
   }
...
void    build_autodig ( CHAR_DATA *ch, char *argument)
{
	if ( !ch->pcdata->autobuild )
	{
        ch->pcdata->autobuild = TRUE;
        send_to_char("{RAUTOBUILD IS ON, BE CARFUL!{x\n\r", ch);
        return;
	} else {
		ch->pcdata->autobuild = FALSE;
        send_to_char("{GAUTOBUILD IS NOW {ROFF{G!{x\n\r", ch);
        return;
	}
}
...
void	build_urooms( CHAR_DATA *ch, char *argument )
{
   ...
   int ftotal = 0;  /* TOTAL number of free rooms */
   int utotal = 0;  /* TOTAL number of used rooms */
...
{
   send_to_char( "No longer needed, done using external program.\n\r", ch );
   return;
}
...
{   
      send_to_char( "Done using external program now. Contact administrator.\n\r", ch );
      return;
}
...
void build_clone( CHAR_DATA *ch, char *argument )
{
   ...
   if ( !str_cmp( arg1, "mob" ) )
   {
	  ...
	  this_mob->hp  			= mob->hp;
	  this_mob->mp  			= mob->mp;
      this_mob->hometown  	    = mob->hometown;

...
int offsetss[4][2] ={ {-2, 0},{ 0, 2},{ 2, 0},{ 0,-2} };
...
const struct map_info_type map_info[] =
{
  ...
  { SECT_UNSEEN, "{x{d"," {x", "", "unknown" },
  { SECT_HERE, "{x{W", "*{x", "", "you!" },
  { SECT_CITY, "{w{i", " {x", "", "city" },
  { SECT_FIELD, "{G", ".{x", "", "field" },
  { SECT_FOREST, "{g", "+", "{g", "forest" },
  { SECT_HILLS,  "{G", "^{x", "{W","hills" },
  { SECT_MOUNTAIN, "{y", "^{x", "", "mountain" },
  { SECT_WATER_SWIM,"{B{i", " {x", "", "shallow water" },
  { SECT_WATER_NOSWIM, "{1", " {x", "", "deep running water" },
  { SECT_RECALL_OK, "{5", " {x", "","inside" },
  { SECT_AIR, "{C{i", " {x", "","air" },
  { SECT_DESERT, "{Y", "^{x", "","desert" },
  { SECT_INSIDE, "{d{i", " {x", "","inside" },
  { SECT_ROAD, "{x", "{Y+", "", "road" },
  { SECT_INN, "{x", "{d+", "", "inn" },
  { SECT_STORE, "{x", "{Y#", "", "store" },
  { SECT_JUNGLE, "{x", "{gY",  "", "jungle" },
  { SECT_BEACH, "{x", "{Y=", "", "beach" },
  { SECT_SWAMP, "{x", "{g~",  "", "swamp" },
  { SECT_TUNDRA, "{x", "{wT",  "", "tundra" },
  { SECT_WASTELAND, "{x", "{WW",  "", "wasteland" },
  { SECT_DENSE_FOREST, "{x", "{GF", "", "dense forest" },
  { SECT_LAVA, "{x", "{R=",  "", "lava" },
  { SECT_ROCK_MOUNTAIN, "{x", "{d^",  "", "rock mountain" },
  { SECT_SNOW_MOUNTAIN, "{x", "{W^",  "", "snow mountain" },
  { SECT_BOG, "{x", "{GB",  "", "bog" },
  { SECT_RUINS, "{x", "{R#",  "", "ruins" },
  { SECT_UNDERGROUND, "{x", "{w#",  "", "underground" },
  { SECT_HOUSE, "{x", "{rH",  "", "house" },
  { SECT_PYRAMID, "{x", "{Y^",  "", "pyramid" },
  { SECT_OCEAN, "{x", "{BO",  "", "ocean" },
  { SECT_STREET, "{x", "{w+", "", "street" },
  { SECT_ALLEY, "{x", "{d+",  "", "alley" },
  { SECT_CAVE, "{x", "{dC",  "", "cave" },
  { SECT_FARMLAND, "{x", "{y:",  "", "farmland" },
  { SECT_CAVERN, "{x", "{dO",  "", "cavern" },
  { SECT_BRIDGE, "{x", "{y=",  "", "bridge" },
  { SECT_ICE, "{x", "{W=",  "", "ice" },
  { SECT_BLOOD_RIVER, "{x", "{R~",  "", "blood river" },
  { SECT_WOODED_ROAD, "{x", "{G+",  "", "wooded road" },
  { SECT_FROZEN_SEA, "{x", "{W~",  "", "frozen sea" },
  { SECT_DOCK, "{x", "{y%",  "", "dock" },
  { SECT_ICEMOUNTAIN, "{x", "{W^",  "", "ice mountain" },
  { SECT_SNOWTREES, "{x", "{W@",  "", "snow trees" },
  { SECT_DIRTPATH, "{x", "{y+",  "", "dirtpath" },
  { SECT_WHEATFIELD, "{x", "{y.", "", "wheatfield" },
  { SECT_DARKWOOD, "{x", "{d@",  "", "darkwood" },
  { SECT_WIND, "{x", "{C~",  "", "wind" },
  { SECT_TORNADO, "{x", "{C@",  "", "tornado" },
  { SECT_WHIRLPOOL, "{x", "{B@",  "", "whirlpool" },
  { SECT_LAVACHUTE, "{x", "{R@",  "", "lavachute" },
  { SECT_EARTH, "{x", "{y~",  "", "earth" },
  { SECT_QUICKSAND, "{x", "{y@",  "", "quicksand" },
  { SECT_CAVE_FLOOR, "{x", "{d=",  "", "cave floor" },
  { SECT_FIRE_ROOM, "{x", "{RX",  "", "fire room" },
  { SECT_DARK_SWAMP, "{x", "{d~",  "", "dark swamp" },
  { SECT_BRICK_ROAD, "{x", "{r+",  "", "brick road" },
...
/* displays seen mobs nearby as numbers */
  ...
  for ( looper = 0,victim = room->first_person;
    ...
    if (  ( can_see( ch, victim ) )
      ...
      looper++;
  if ( looper > 0 )
    sprintf( contents[x][y].string, "%i{x", UMIN( looper, 9 ) );
  else
               ...
               && ( !str_cmp( pexit->keyword, "" ) )  )  ) )
      {
        ...
        if (  ( door_type < DOOR_CLOSED )
           && (  ( line_of_sight == LOS_INITIAL )
              || ( door == line_of_sight )  )
           && (map[x+offsetss[door][0]][y+offsetss[door][1]]==SECT_UNSEEN) )
        {
          MapAreat (pexit->to_room,ch,
                    x+offsetss[door][0], y+offsetss[door][1],min,max, 
           ( line_of_sight == LOS_INITIAL ) ?
             door :
             line_of_sight );
        }
...
void ShowMapt( CHAR_DATA *ch, int min, int max, int size, int center )
{
  int x,y, looper;
  char outbuf[MSL];
  char catbuf[MSL];
...
/* starts the mapping with the center room */
  MapAreat(ch->in_room, ch, center, center, min-1, max, LOS_INITIAL); 

/* marks the center, where ch is */
  sprintf( contents[center][center].string, "%s", "{f*{x" ); 

  if ( size == 7 )
    ShowRoom( ch, min, max, size, center ); 
  else
    ShowMapt( ch, min, max, size, center );
  return;
}
/* For use with ITEM_MAP stuff.
 */
void item_mapper( CHAR_DATA *ch, OBJ_DATA *obj )
{
  int size,center,x,y,min,max,looper;
  size = 7;
...
/* starts the mapping with the center room */
  MapAreat(get_room_index(obj->value[0]), ch, center, center, min-1, max, LOS_INITIAL);

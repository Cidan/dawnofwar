...
int rclass_lookup (const char *name)
{
 
   return -1;
}

int reclass_lookup (const char *name, CHAR_DATA *ch)
{
...
int classt_lookup (const char *name, CHAR_DATA *ch)
{
...
void my_get_age( CHAR_DATA *ch, char *buf )
{
    ...
    sprintf( buf+strlen(buf), "%4d years, %2d months and %2d days", 
       years, months, days );
    return;
}
...
int get_curr_lck( CHAR_DATA *ch )
{
    ...
    max = ch->pcdata->max_lck;

    return URANGE( 3, ch->pcdata->perm_lck + ch->pcdata->mod_lck, max );
}
...
/*
    if ( ( wield = get_eq_char( ch, WEAR_HOLD_HAND_L ) ) != NULL && !IS_WEAPON(wield) )
     {
      ch->hitroll += wield->value[1];
      ch->damroll += wield->value[2];
     }

    if ( ( wield = get_eq_char( ch, WEAR_HOLD_HAND_R ) ) != NULL && !IS_WEAPON(wield) )
     {
      ch->hitroll += wield->value[1];
      ch->damroll += wield->value[2];
     }
...
*/
   ...
   SET_BIT( room->affected_by, raf->bitvector );
	...
	sprintf( buf, "{R%s{x has cast {d%s{x in {xarea: {G%s{x, {xroom: {G%d{x.",
...
/* Ramias... Don't copy uninitialized fields: next, prev, is_free */
    ...
    for (this_aff = ch->first_affect; this_aff != NULL; this_aff = this_aff->next)
    {
        if (this_aff->type == paf->type)
        {
            this_aff->duration = paf->duration;
            return;
        }
    }
...
void char_to_room( CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex )
{
    ...
    {
      send_to_char( "{xAs you step into the room, you fleetingly see a mystical {RFire{x Rune suspended in front of you, which then {REXPLODES{x!!!\n\r", ch );
      act( "{xThe {RFire{x Rune explodes as $n enters the room!", ch, NULL, NULL, TO_ROOM );
    ...
    if (  IS_SET( ch->in_room->affected_by, ROOM_BV_SHOCK_RUNE ) 
       && ( ch->is_free == FALSE )  )
    {
      send_to_char( "{xAs you step into the room, you fleetingly see a mystical {BShock{x Rune suspended in front of you, which then {BZAPS{x You!!!\n\r", ch );
      act( "{xThe {BShock{x Rune flashes as $n enters the room!", ch, NULL, NULL, TO_ROOM );
    ...
    if ( IS_SET( ch->in_room->affected_by, ROOM_BV_POISON_RUNE ) 
       && ( ch->is_free == FALSE )  )
    {
      send_to_char( "{xAs you step into the room, you fleetingly see a mystical {dPoison{x Rune suspended in front of you, which then {dEXPLODES{x!!!\n\r", ch );
      act( "{xThe {dPoison{x Rune explodes as $n enters the room!", ch, NULL, NULL, TO_ROOM );
...
void equip_char( CHAR_DATA *ch, OBJ_DATA *obj, int iWear )
{
    ...
    if ( IS_SET( obj->item_apply, ITEM_APPLY_INFRA )
		&& ( !IS_NPC(ch) && ch->desc->connected != CON_SETTING_STATS) )
    {
...
void unequip_char( CHAR_DATA *ch, OBJ_DATA *obj )
{
    ...
    */
    ...
    if ( IS_SET( obj->item_apply, ITEM_APPLY_INFRA ) && ch->is_quitting == FALSE )
    {
       act( "$n's eyes become dim.", ch, NULL, NULL, TO_ROOM );
       send_to_char( "Your eyes become dim!\n\r", ch );
    }
    
    if ( IS_SET( obj->item_apply, ITEM_APPLY_INV ) && ch->is_quitting == FALSE )
    {
       act( "$n slowly fades back into existance.", ch, NULL, NULL, TO_ROOM );
       send_to_char( "You slowly fade into existance.\n\r", ch );
    }
    
    if ( IS_SET( obj->item_apply, ITEM_APPLY_DET_INV ) && ch->is_quitting == FALSE )
       send_to_char( "You feel less aware of your surroundings.\n\r", ch );
    
    if ( IS_SET( obj->item_apply, ITEM_APPLY_SANC ) && ch->is_quitting == FALSE )
    {
       act( "The white aura around $n fades.", ch, NULL, NULL, TO_ROOM );
       send_to_char( "The white aura around you fades.\n\r", ch );
    }
    
    if ( IS_SET( obj->item_apply, ITEM_APPLY_SNEAK ) && ch->is_quitting == FALSE )
       send_to_char( "You feel less sneaky!\n\r", ch );
    
    if ( IS_SET( obj->item_apply, ITEM_APPLY_HIDE ) && ch->is_quitting == FALSE )
       send_to_char( "You feel less hidden.\n\r", ch );
    
    if ( IS_SET( obj->item_apply, ITEM_APPLY_PROT ) && ch->is_quitting == FALSE )
       send_to_char( "You feel less protected.\n\r", ch );
    
    if ( IS_SET( obj->item_apply, ITEM_APPLY_ENHANCED ) && ch->is_quitting == FALSE )
    ...
    if ( IS_SET( obj->item_apply, ITEM_APPLY_PASS_DOOR ) && ch->is_quitting == FALSE )
    {
       act( "$n becomes solid again.", ch, NULL, NULL, TO_ROOM );
       send_to_char( "You feel more solid!\n\r", ch );
    }
    
    if ( IS_SET( obj->item_apply, ITEM_APPLY_FLY ) && ch->is_quitting == FALSE )
    {
...
CHAR_DATA *get_char_room( CHAR_DATA *ch, char *argument )
{
	...
	if ( !can_see( ch, rch ) )
      ...
      if ( (!IS_NPC(ch) && !IS_NPC(rch) && !IS_IMMORTAL(ch) && !IS_IMMORTAL(rch)) && 
          (rch->hometown != ch->hometown) )
/* 1 */    {
            if ( rch->level < 50)  
/* 2 */     {
               if ( is_name( arg, race_table[rch->race].race_title ) )
/* 3 */            {
                   if ( ++count == number )
	               return rch;
                 }
      ...
      if ( ( rch->level >= 50 || rch->hometown == ch->hometown || IS_IMMORTAL(ch) || IS_NPC(ch) || IS_NPC(rch) ) )
       {
...
CHAR_DATA *get_char_connected( CHAR_DATA *ch, char *argument )
{
    char arg[MAX_INPUT_LENGTH];
    CHAR_DATA *wch;
    int number;
...
OBJ_DATA *get_obj_stored( CHAR_DATA *ch, char *argument )
{
    char arg[MAX_INPUT_LENGTH];
    OBJ_DATA *obj;
    int number;
...
bool can_see_sobj( CHAR_DATA *ch, OBJ_DATA *obj )
{
	if (!obj->stored)
		return FALSE;
    if ( !IS_NPC(ch) && IS_SET(ch->act, PLR_HOLYLIGHT) )

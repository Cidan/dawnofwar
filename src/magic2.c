...
bool spell_locate_object( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
	...
	if ( in_obj->carried_by != NULL )
	{
		...
		PERSON(ch, in_obj->carried_by) : "Someone");
	}
	else
	{
            	found = TRUE;
...
bool spell_summon( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
	...
	return TRUE;
       ...
       if ( IS_AFFECTED(victim, AFF_NOQUIT) && !IS_IMMORTAL(ch) )
       {
        send_to_char( "You failed.\n\r", ch );
        return TRUE;
       }
        ...
        switch ( obj_lose->item_type )
        {
...

          {
            ...
            act( "$p fumes and dissolves!",
              victim, obj_lose, NULL, TO_CHAR );
            for( content = obj_lose->first_in_carry_list;
                 content;
...
bool spell_fire_breath( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
    ...
    {
      OREF( obj_next, OBJ_NEXTCONTENT );
      for ( obj_lose = victim->first_carry;
            obj_lose != NULL;
            obj_lose = obj_next )
      {
        char *msg;
...
bool spell_planergy( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
    ...
    int plane;
	 ...
	 if ( level<=  8 ) { lvl =  4; plane=REALM_IMPACT;        } 
    else if ( level<= 17 ) { lvl = 13; plane=REALM_GAS;   }
    else if ( level<= 26 ) { lvl = 22; plane=REALM_FIRE;         }
    else if ( level<= 35 ) { lvl = 31; plane=REALM_LIGHT;       }
    else if ( level<= 44 ) { lvl = 40; plane=REALM_SHOCK;    }
    else if ( level<= 53 ) { lvl = 49; plane=REALM_SOUND;  }
    else if ( level<= 62 ) { lvl = 58; plane=REALM_MIND;       }
    else if ( level<= 71 ) { lvl = 67; plane=REALM_ACID;    }
      else                 { lvl = 80; plane=REALM_POISON;      }
...
bool spell_nerve_fire( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
    ...
    for ( vch = first_char; vch != NULL; vch = vch_next )
    {
      ...
      if ( vch->in_room == ch->in_room )
      {
        if ( vch != ch && ( IS_NPC(ch) ? !IS_NPC(vch) : IS_NPC(vch) ) )
        {
          sp_damage( obj,  ch, vch, (level) + dice(5, 20),
            REALM_MIND | NO_REFLECT | NO_ABSORB, sn, TRUE );
        }
        continue;
      }

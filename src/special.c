...
SPEC_FUN *spec_lookup( const char *name )
{
    ...
    if ( !str_cmp( name, "spec_guard"             ) ) return spec_policeman;
    ...
    if ( !str_cmp( name, "spec_policeman"         ) ) return spec_policeman;
    if ( !str_cmp( name, "spec_cast_cadaver"      ) ) return spec_cast_cadaver;
    if ( !str_cmp( name, "spec_undead"            ) ) return spec_undead;
    if ( !str_cmp( name, "spec_stephen"           ) ) return spec_stephen;
    if ( !str_cmp( name, "spec_rewield"           ) ) return spec_rewield;
    if ( !str_cmp( name, "spec_sylai_priest"      ) ) return spec_sylai_priest;
    if ( !str_cmp( name, "spec_cast_bigtime"	  ) ) return spec_cast_bigtime;
    if ( !str_cmp( name, "spec_sage"		  ) ) return spec_sage;
    if ( !str_cmp( name, "spec_wizardofoz"        ) ) return spec_wizardofoz;
    if ( !str_cmp( name, "spec_vamp_hunter"	  ) ) return spec_vamp_hunter;
    if ( !str_cmp( name, "spec_mino_guard"	  ) ) return spec_mino_guard;
    if ( !str_cmp( name, "spec_tax_man"		  ) ) return spec_tax_man;
    if ( !str_cmp( name, "spec_thant_guard"	  ) ) return spec_thant_guard;
    if ( !str_cmp( name, "spec_thant_road"	  ) ) return spec_thant_road;
    if ( !str_cmp( name, "spec_azt_guard"	        ) ) return spec_azt_guard;
    if ( !str_cmp( name, "spec_azt_road"	        ) ) return spec_azt_road;
    if ( !str_cmp( name, "spec_lum_guard"	        ) ) return spec_lum_guard;
    if ( !str_cmp( name, "spec_lum_road"	        ) ) return spec_lum_road;
    if ( !str_cmp( name, "spec_thant_scout"	        ) ) return spec_thant_scout;
...
{
    if ( func == spec_breath_any        ) return "spec_breath_any";
    if ( func == spec_breath_acid       ) return "spec_breath_acid";
    if ( func == spec_breath_fire       ) return "spec_breath_fire";
    if ( func == spec_breath_frost      ) return "spec_breath_frost";
    if ( func == spec_breath_gas        ) return "spec_breath_gas";
    if ( func == spec_breath_lightning  )return "spec_breath_lightning";
    if ( func == spec_cast_adept        ) return "spec_cast_adept";
    if ( func == spec_cast_cleric       ) return "spec_cast_cleric";
    if ( func == spec_cast_judge        ) return "spec_cast_judge";
    if ( func == spec_cast_mage         ) return "spec_cast_mage";
    if ( func == spec_cast_undead       ) return "spec_cast_undead";
    if ( func == spec_executioner       ) return "spec_executioner";
    if ( func == spec_fido              ) return "spec_fido";
    if ( func == spec_guard             ) return "spec_policeman";
    if ( func == spec_janitor           ) return "spec_janitor";
    if ( func == spec_mayor             ) return "spec_mayor";
    if ( func == spec_poison            ) return "spec_poison";
    if ( func == spec_thief             ) return "spec_thief";
    if ( func == spec_policeman         ) return "spec_policeman";
    if ( func == spec_cast_cadaver      ) return "spec_cast_cadaver";
    if ( func == spec_undead            ) return "spec_undead";
    if ( func == spec_stephen           ) return "spec_stephen";
    if ( func == spec_rewield           ) return "spec_rewield";
    if ( func == spec_sylai_priest      ) return "spec_sylai_priest";
    if ( func == spec_cast_bigtime	) return "spec_cast_bigtime";
    if ( func == spec_sage		) return "spec_sage";
    if ( func == spec_wizardofoz        ) return "spec_wizardofoz";
    if ( func == spec_vamp_hunter	)	return "spec_vamp_hunter";
    if ( func == spec_mino_guard	)	return "spec_mino_guard";
    if ( func == spec_tax_man		)	return "spec_tax_man";
    if ( func == spec_thant_guard		)	return "spec_thant_guard";
    if ( func == spec_thant_road		)	return "spec_thant_road";
    if ( func == spec_azt_guard		)	return "spec_azt_guard";
    if ( func == spec_azt_road		)	return "spec_azt_road";
    if ( func == spec_lum_guard		)	return "spec_lum_guard";
    if ( func == spec_lum_road		)	return "spec_lum_road";
    if ( func == spec_thant_scout		)	return "spec_thant_scout";
...
strcat( buf, "       spec_thant_guard \n\r" );
strcat( buf, "       spec_thant_road \n\r" );
strcat( buf, "       spec_azt_guard \n\r" );
strcat( buf, "       spec_azt_road \n\r" );
strcat( buf, "       spec_lum_guard \n\r" );
strcat( buf, "       spec_lum_road \n\r" );
strcat( buf, "       spec_thant_scout \n\r" );
...
bool spec_executioner( CHAR_DATA *ch )
{
	...
	if ( !IS_NPC(victim) && IS_SET(victim->act, PLR_THIEF) )
        ...
        if ( IS_NPC(victim) && IS_SET(victim->act, ACT_UNDEAD) )
            { crime = "UNDEAD"; undead = TRUE; break; } 
...
bool spec_mino_guard( CHAR_DATA *ch )
{
	...
	if (  (  (  ( ( pass = get_eq_char( victim, WEAR_HOLD_HAND_R ) ) != NULL )
        ...
        || (  ( ( pass = get_eq_char( victim, WEAR_HOLD_HAND_L ) ) != NULL )
           && ( pass->pIndexData->vnum == OBJ_VNUM_MINO_PASS )  ) )
        {
          do_say( ch, "{RLet this one pass{x" );
          continue;
        }
...
bool spec_fido( CHAR_DATA *ch )
{
    ...
    for ( corpse = ch->in_room->first_content; corpse != NULL;
          corpse = corpse->next_in_room )
    {
...
bool spec_janitor( CHAR_DATA *ch )
{
    ...
    for ( trash = ch->in_room->first_content; trash != NULL;
          trash = trash->next_in_room )
    {
...
/*    if ( (int) (ch->hunting) > 0)*/ /* Don't hunt someone if already hunting */
	return FALSE;

     
    sprintf( buf, "%s is a %s, I shall not rest till justice is done!", 
	     victim->name, crime );
    do_yell( ch, buf );
...
bool spec_undead( CHAR_DATA *ch )
{
	...
	case  1: min_level =  3; spell = "chill touch";    break;
	case  2: min_level =  7; spell = "weaken";         break;
	case  3: min_level = 30; spell = "earthquake";     break;
	case  4: min_level = 60; spell = "dispel magic";   break;
	case  5: min_level = 12; spell = "curse";          break;
	case  6: min_level = 20; spell = "energy drain";   break;
	case  7: min_level = 40; spell = "gate";           break;
	case  8: min_level = 30; spell = "fireball";       break;
	default: min_level =  1; spell = "summon";         break;
	}
...
bool spec_sylai_priest( CHAR_DATA *ch )
{
    ...
    */
    ...
    act( "$n lands on the floor, having fallen from the outcrop above!",
...
bool spec_cast_bigtime( CHAR_DATA *ch )
{
	...
	switch ( number_bits( 4 ) )
	{
	case  0: min_level = 45; spell = "dispel magic";	break;
	case  1: min_level = 46; spell = "acid blast";		break;
	case  2: min_level = 30; spell = "phobia";		break;
	case  3: min_level = 55; spell = "chain lightning";	break;
	case  4: min_level = 65; spell = "energy drain";        break;
	case  5: min_level = 43; spell = "mindflame";   	break;
	case  6: min_level = 12; spell = "lava burst";         break;
	case  7: min_level = 44; spell = "static";              break;
	default: min_level = 25; spell = "hellspawn";           break;
	}
...
bool spec_sage( CHAR_DATA *ch )
{
    ...
    static const char open_path[] =
	"WDDDHDFDCUVD1XDD2YDD3DD4DFD5DGDD6D7XDD7YDD8DFD9XDDDBDFD.";
  ...
  switch ( number_range( 1 , 21 ) )
  {
    ...
    case 6 :
		 sprintf( buf, "%s", ch->hunting->name );
             sprintf( buf1, "{RI know that you are a Vampyre, and I shall not rest until your are destroyed!!!{x\n\r" );
    ...
    case 17 :
		 sprintf( buf, " %s ", ch->hunting->name );
             sprintf( buf1, "Do you finally know fear? I shall not rest until ALL of your kind are destroyed!!!{x\n\r" );
             safe_strcat( MAX_STRING_LENGTH, buf, buf1 );
             do_tell( ch, buf );
             break;
    ...
    case 19 : break;
    case 20 :
              sprintf ( buf, "All the realm shall know that %s is a Vampyre, and I shall eridicate all of these vile creatures!!\n\r", ch->hunting->name );
              do_yell( ch, buf );
              break;
    default : break;
...
/*       break;
    ...
    act( "$n says, 'Since you refuse to pay what you owe, it has automatically been deducted from your funds'.", ch, NULL, victim, TO_ROOM );
...
bool spec_thant_guard( CHAR_DATA *ch )
{
    char buf[MAX_STRING_LENGTH];
    CHAR_DATA *victim;
    CHAR_DATA *pch;
    ...
    if ( IS_IMMORTAL(victim) )
      continue;
        ...
        if ( victim->hometown != 1 && victim->hometown != 0 )
          {
            sprintf ( buf, "GUARDS!! I HAVE %s CORNERED AT %s!!",
                ( IS_NPC(victim) ? victim->short_descr : get_pkn(victim) ), ch->in_room->name );   
                do_yell( ch, buf );
                multi_hit( ch, victim, TYPE_UNDEFINED );
         ...
         for ( pch = first_char; pch != NULL; pch = pch->next )
          {
                  ...
                  &&   !(str_cmp (rev_spec_lookup(pch->spec_fun), "spec_thant_road") ) )
                   {
                    set_hunt(pch,NULL,victim,NULL,HUNT_OPENDOOR|HUNT_PICKDOOR|HUNT_UNLOCKDOOR|HUNT_INFORM,HUNT_CR); //|HUNT_WORLD
...
bool spec_thant_road( CHAR_DATA *ch )
{
    char buf[MAX_STRING_LENGTH];
    CHAR_DATA *victim;
    CHAR_DATA *pch;
         ...
         for ( pch = first_char; pch != NULL; pch = pch->next )
          {
                  ...
                  &&   !(str_cmp (rev_spec_lookup(pch->spec_fun), "spec_thant_road") ) )
                   {
                    set_hunt(pch,NULL,victim,NULL,HUNT_OPENDOOR|HUNT_PICKDOOR|HUNT_UNLOCKDOOR|HUNT_INFORM,HUNT_CR);
...
bool spec_azt_guard( CHAR_DATA *ch )
{
    char buf[MAX_STRING_LENGTH];
    CHAR_DATA *victim;
    CHAR_DATA *pch;
    ...
    if ( IS_IMMORTAL(victim) )
      continue;
        ...
        if ( victim->hometown != ( 2 || 0 ) )
          {
    sprintf ( buf, "GUARDS!! I HAVE %s CORNERED AT %s!! SWARM SWARM!!", 
        ( IS_NPC(victim) ? victim->short_descr : get_pkn(victim) ), ch->in_room->name );   
    do_yell( ch, buf );
    multi_hit( ch, victim, TYPE_UNDEFINED );
         ...
         for ( pch = first_char; pch != NULL; pch = pch->next )
          {
      		...
      		&&   IS_AWAKE(pch)
                  &&   pch->in_room != NULL
                  &&   pch->in_room->area == ch->in_room->area
                  &&   !(str_cmp (rev_spec_lookup(pch->spec_fun), "spec_azt_road") ) )
                   {
                    set_hunt(pch,NULL,victim,NULL,HUNT_OPENDOOR|HUNT_PICKDOOR|HUNT_UNLOCKDOOR|HUNT_INFORM,HUNT_CR);
...
bool spec_azt_road( CHAR_DATA *ch )
{
    char buf[MAX_STRING_LENGTH];
    CHAR_DATA *victim;
    CHAR_DATA *pch;
...
bool spec_lum_guard( CHAR_DATA *ch )
{
    char buf[MAX_STRING_LENGTH];
    CHAR_DATA *victim;
    CHAR_DATA *pch;
    ...
    if (  IS_IMMORTAL(victim)  )
      continue;
        ...
        if ( victim->hometown != ( 3 || 0 ) )
          {
    sprintf ( buf, "Requesting backup by all guard untis at %s. Citizens please move away from %s and let us handle it.",
        ch->in_room->name, ( IS_NPC(victim) ? victim->short_descr : get_pkn(victim) ) );   
    do_yell( ch, buf );
    multi_hit( ch, victim, TYPE_UNDEFINED );
         ...
         for ( pch = first_char; pch != NULL; pch = pch->next )
          {
      		...
      		&&   IS_AWAKE(pch)
                  &&   pch->in_room != NULL
                  &&   pch->in_room->area == ch->in_room->area
                  &&   !(str_cmp (rev_spec_lookup(pch->spec_fun), "spec_lum_road") ) )
                   {
                    set_hunt(pch,NULL,victim,NULL,HUNT_OPENDOOR|HUNT_PICKDOOR|HUNT_UNLOCKDOOR|HUNT_INFORM,HUNT_CR);
                  }
            }
...
bool spec_lum_road( CHAR_DATA *ch )
{
    char buf[MAX_STRING_LENGTH];
    CHAR_DATA *victim;
    CHAR_DATA *pch;
...
bool spec_thant_scout( CHAR_DATA *ch )
{
    char buf[MAX_STRING_LENGTH];
    CHAR_DATA *victim;
    CHAR_DATA *pch;
    ...
    if ( !IS_AWAKE(ch) || ch->fighting != NULL )
	return FALSE;
    /* If he's not at his starting post but is in the target room...
     * Yell and alert everyone in town as to where he saw the enemy.
     *
     */
    if (ch->in_room != NULL &&
        ch->hunt_dest != NULL &&
        ch->in_room->vnum == ch->hunt_dest->vnum &&
        ch->going_to_town == TRUE)
    {
      end_hunt(ch);
        sprintf(buf, "I SPOTTED %s AT %s AND CAME TO REPORT IT! RETURNING TO MY POST!", get_pkn(ch->spotted),
            ch->was_in_room_guard->name);
        do_yell( ch, buf);
      ch->hunt_home = ch->was_in_room_guard;
      ch->hunt_dest = NULL;
      ch->was_in_room_guard = NULL;
      ch->going_to_town = FALSE;
      ch->spotted = NULL;
      ch->going_to_post = TRUE;
      ch->hunt_flags = HUNT_OPENDOOR|HUNT_PICKDOOR|HUNT_UNLOCKDOOR|HUNT_WORLD;
      mob_hunt(ch);
      return TRUE;
    }

    if (ch->going_to_town == TRUE)
        return FALSE;
    if (ch->going_to_post == TRUE)
        return FALSE;

    /* Look for everyone in room */
    for ( victim = ch->in_room->first_person; victim != NULL;
          victim = victim->next_in_room )
    {
    ...
    if ( IS_NPC(victim) || IS_IMMORTAL(victim)  )
      continue;
    /* Can't let the guard stop halfway before he reports an enemy!
     */
        if (ch->going_to_town == TRUE)
            continue;
        if (ch->going_to_post == TRUE)
        ...
        if (victim->hometown == ( 1 || 0 ) )
            continue;
          sprintf ( buf, "%s SPOTTED AT %s! I MUST GO WARN THE TOWN!",get_pkn(victim),
                ch->in_room->name);   
    do_yell( ch, buf );
    ch->was_in_room_guard = ch->in_room;
    ch->spotted = victim;
    ch->going_to_town = TRUE;
    ch->hunt_home = get_room_index(ch->report_room);
    ch->hunt_dest = ch->hunt_home;
    ch->hunt_flags = HUNT_OPENDOOR|HUNT_PICKDOOR|HUNT_UNLOCKDOOR|HUNT_WORLD;
    mob_hunt(ch);

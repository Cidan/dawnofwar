...
bool spell_embrace(int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
    ...
    {
      send_to_char( "{dRENEGADES{x can't embrace!!!\n\r", ch );
      return FALSE;
    }

    if ( IS_VAMP( victim ) )
     {
       send_to_char( "{xYour {dchosen{x has already been {REmbraced{x!!!.\n\r", ch );
       return FALSE;
     }

    if ( IS_WOLF( victim ) )
    {
      send_to_char( "A sudden awareness comes over you, as you peer into their soul and see a {yGarou{x!!!\n\r", ch );
      if ( !IS_AWAKE( victim ) )
        do_wake( victim, "" );
...
/*    if ( victim->pcdata->clan == 7 )
     {
       send_to_char( "{mNO WAY!!!!!{W They are {dKindred {RKillers!!!!!{x\n\r", ch );
       return FALSE;
     }
*/
     if ( victim->position > POS_SLEEPING ) 
   {
...

     {
       ...
       send_to_char( "{xYou sink your fangs into your helpless victim.\n\r", ch );
       send_to_char( "Then, in the final death throes, you bite your own arm and allow the {Rblood{x\n\r", ch );
       send_to_char( "to drip into their mouth, damning them to {dETERNAL NIGHT{x!!!\n\r", ch );
       send_to_char( "you feel some of your essential {Rblood{x potential drain into your victim!\n\r", ch );
       send_to_char( "{xYou feel your soul ripped asunder as the Vampyre draws your lifeblood into it's mouth!!\n\r", victim );
       send_to_char( "As your last breath escapes you, and you begin to submit to death itself, you suddenly feel\n\r", victim );
       send_to_char( "a hot, coppery tasting fluid enter your mouth.  As the strength of the {dKindred{x begins to fill\n\r", victim );
       send_to_char( "your veins, your mind is flooded with the sudden knowledge of damnation and {dEternal Night{x!!!\n\r", victim );
       send_to_char( "Awakening to a new knowledge of your status as a {RChosen{x, you know that upon your\n\r", victim );
       send_to_char( "first taste of mortal {Rblood{x, you shall forever enter the ranks of the {dKindred{x!!\n\r", victim );
       send_to_char( "A choice awaits you....shall you accept this destiny and feed upon mortals, or shall you\n\r", victim );
       send_to_char( "seek {Wcleansing{x from this {dEternal Damnation{x???\n\r", victim );
...
void do_family( CHAR_DATA *ch, char *argument )
{
   ...
   if ( ch->level < L_GOD )
      ...
      for ( index = 0; index <= 4 ; index++ )
      {
        ...
        for ( d = first_desc; d!=NULL; d = d->next )
        {
            ...
            &&   victim->in_room != NULL )
            {
                ...
                sprintf( buf, "%-15s {xGen: {G%d   {xRank: {d%d{x  {xBloodlust: {R%d{x/{R%d{x   %-15s\n\r",
                  victim->name, victim->pcdata->generation, victim->pcdata->vamp_level, victim->pcdata->bloodlust, victim->pcdata->bloodlust_max, victim->in_room->name );
              send_to_char( buf, ch );
             }
         }
    
        if ( !found )
            send_to_char( "{xNo {dKindred{x Family members were found.\n\r", ch );
    ...
    else
    {
      ...
      sprintf( buf, "{WMembers of the {dKindred {xFamily %s\n\r", get_family_name( ch ) );
      send_to_char( buf, ch );
      found = FALSE;
      for ( d = first_desc; d!=NULL; d = d->next )
      {
        if ( d->connected == CON_PLAYING
          ...
          && !IS_IMMORTAL( victim )   )
          {
              ...
              found = TRUE;
              sprintf( buf, "%-15s {xGeneration: {G%d     {xRank: {d%d{x\n\r",
       ...
       if ( !found )
          send_to_char( "{xNo other {dKindred {x Family members were found.\n\r", ch );
 ...
 void do_instruct( CHAR_DATA *ch, char *argument )
 {
   ...
   if ( !IS_VAMP( victim ) )
   {
     send_to_char( "{RWHAT????{x You want to teach {dKindred{x skills to someone not {RCHOSEN{x? NO WAY!!", ch );
     return;
   }
...
/* okay, we know the victim is good, and a vamp..check for skill known, levels, etc. */
   ...
   {
     send_to_char( "{xYou don't know that {dKindred{x skill!.\n\r", ch );
     return;
   }
...
/*  FIX ME
   ...
   {
     send_to_char("{xThey are too inexperienced in the ways of the {dKindred{x to learn this skill.\n\r", ch );
     send_to_char( "{xYou are to inexperienced in the ways of the {dKindred{x to learn this skill.\n\r", victim );
     return;
   }
*/
   if ( victim->pcdata->vamp_skill_num >= victim->pcdata->vamp_skill_max )
   {
     ...
     send_to_char( "{xYou are unable to learn any more {dKindred{x knowledge at this time.\n\r", victim );
     return;
   }
...
/* Okay, the skill is good, the instructor knows it, and the victim doesn't  */
   ...
   send_to_char( "You have taught them another way of the {dKindred{x!!!\n\r", ch );
   sprintf( buf, "You are now learned in the way of {R%s{x!!!\n\r", skill_table[sn].name );
...
bool spell_blood_walk( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
    ...
    {
	send_to_char( "Your {Rblood{x burns with rage, as your efforts are shaken off.\n\r", ch );
	return TRUE;
    }
    if ( ( time_info.hour < 19 ) && ( time_info.hour > 5 ) ) 
    {
      send_to_char( "{xYou may only {Rblood walk{x at night!!\n\r", ch );
      return FALSE;
    }
    ...
    {
       send_to_char( "You cannot sense your target's {Rblood{x!\n\r", ch );
       return TRUE;
    }
    ...
    send_to_char( "{xYou feel yourself drawn to the {Rblood{x within your victim!\n\r", ch );
    do_look( ch, "auto" );
    return TRUE;
}
...
bool spell_blood_sign(int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
  ...
  {
    case 0 : costring = "{C"; break;
    case 1 : costring = "{c"; break;
    case 2 : costring = "{r"; break;
    case 3 : costring = "{m"; break;
    case 4 : costring = "{R"; break;
...
bool spell_blood_sense(int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
  ...
  {
    send_to_char( "You do not sense any {RBloodSign{x here.\n\r", ch );
    return FALSE;
  }

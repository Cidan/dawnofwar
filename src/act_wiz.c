...
#include "ack.h"
#include "tables.h"
#include "hash.h"  /* Clan stuff, since they are dynamic now */
#ifndef DEC_EMAIL_H
#include "email.h"
#endif
...
void do_wizhelp( CHAR_DATA *ch, char *argument )
{
 ...
 if (ch->level == L_GOD || get_trust(ch) == L_GOD)
     {
      send_to_char( "\n\r{WLevel 110:\n\r\n\r{x", ch);
	...
	{
	    sprintf( buf, "{c%-12s{x", cmd_table[cmd].name );
	    strcat( buf1, buf );
	    if ( ++col % 6 == 0 )
		strcat( buf1, "\n\r" );
	}
 ...
 if (ch->level >= 109 || get_trust(ch) >= 109)
     {
      send_to_char( "\n\r{WLevel 109:\n\r\n\r{x", ch);

    for ( cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++ )
    {
	   if ( cmd_table[cmd].level == 109 )
	{
	    sprintf( buf, "{c%-12s{x", cmd_table[cmd].name );
	    strcat( buf1, buf );
	    if ( ++col % 6 == 0 )
 ...
 if (ch->level >= 108 || get_trust(ch) >= 108)
     {
      send_to_char( "\n\r{WLevel 108:\n\r\n\r{x", ch);
 ...
 if (ch->level >= 107 || get_trust(ch) >= 107)
     {
      send_to_char( "\n\r{WLevel 107:\n\r\n\r{x", ch);

    for ( cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++ )
    {
	   if ( cmd_table[cmd].level == 107 )
	{
	    sprintf( buf, "{c%-12s{x", cmd_table[cmd].name );
	    strcat( buf1, buf );
	    if ( ++col % 6 == 0 )
...
if (ch->level >= 106 || get_trust(ch) >= 106)
     {
      send_to_char( "\n\r{WLevel 106:\n\r\n\r{x", ch);

    for ( cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++ )
    {
	   if ( cmd_table[cmd].level == 106 )
	{
	    sprintf( buf, "{c%-12s{x", cmd_table[cmd].name );
	    strcat( buf1, buf );
	    if ( ++col % 6 == 0 )
	...
	if (ch->level >= 105 || get_trust(ch) >= 105)
     {
      send_to_char( "\n\r{WLevel 105:\n\r\n\r{x", ch);

    for ( cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++ )
    {
	   if ( cmd_table[cmd].level == 105 )
	{
	    sprintf( buf, "{c%-12s{x", cmd_table[cmd].name );
	    strcat( buf1, buf );
	    if ( ++col % 6 == 0 )
	...
	if (ch->level >= 104 || get_trust(ch) >= 104)
     {
      send_to_char( "\n\r{WLevel 104:\n\r\n\r{x", ch);

    for ( cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++ )
    {
	   if ( cmd_table[cmd].level == 104 )
	{
	    sprintf( buf, "{c%-12s{x", cmd_table[cmd].name );
	    strcat( buf1, buf );
	    if ( ++col % 6 == 0 )
	...
	if (ch->level >= 103 || get_trust(ch) >= 103)
     {
      send_to_char( "\n\r{WLevel 103:\n\r\n\r{x", ch);

    for ( cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++ )
    {
	   if ( cmd_table[cmd].level == 103 )
	{
	    sprintf( buf, "{c%-12s{x", cmd_table[cmd].name );
	    strcat( buf1, buf );
	    if ( ++col % 6 == 0 )
	...
	if (ch->level >= 102 || get_trust(ch) >= 102)
     {
      send_to_char( "\n\r{WLevel 102:\n\r\n\r{x", ch);

    for ( cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++ )
    {
	   if ( cmd_table[cmd].level == 102 )
	{
	    sprintf( buf, "{c%-12s{x", cmd_table[cmd].name );
	    strcat( buf1, buf );
	    if ( ++col % 6 == 0 )
...
//    bool found = FALSE;
   ...
   victim = get_char_world( ch, arg1 );
    ...
    if ( ( victim->level >= ch->level ) && IS_IMMORTAL ( victim ) )
    {
    send_to_char( "Transfer someone who's on or above your level? HA! Nice try.\n\r", ch );
    return;
    }
...
void do_rstat( CHAR_DATA *ch, char *argument )
{
	...
	location->vnum,
	location->light, 
	rev_table_lookup( tab_sector_typess, location->sector_type ) );
    strcat( buf1, buf );

    sprintf( buf,
	"Room flags: %s.\n\rDescription:\n\r%s",
	bit_table_lookup( tab_room_flags, location->room_flags ),
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
	...
	get_curr_dex(victim), victim->pcdata->max_dex,
	get_curr_con(victim), victim->pcdata->max_con );
    strcat( buf1, buf );
    }
     ...
     if ( !IS_NPC(victim) )
{
    ...
    if ( !IS_NPC( victim ) )
    {
	  ...
	  victim->pcdata->clan,
	  get_clan_info(victim->pcdata->clan, CABBR),
	  victim->balance );
       strcat( buf1, buf );
    }
    ...
    sprintf( buf, "Carry number: %d.  Carry weight: %4.2f.   {CQuest Points{W: {Y%3d{x\n\r",
...
/*    if ( IS_NPC( victim ) 
    ...
    {
       sprintf( buf, "{gMoving to room vnum: (%d) %s.{w\n\r", victim->move_to,
       victim->movename );
       strcat( buf1, buf );
    }*/

    strcpy(buf, "{gMoving to room vnum: (%d) %s.{w\n\r");
    if ( victim->hunting )
      sprintf(buf1+strlen(buf1), buf, victim->hunting->in_room->vnum,
              victim->hunting->in_room->name);
    else if ( victim->hunt_obj && victim->hunt_obj->in_room )
      sprintf(buf1+strlen(buf1), buf, victim->hunt_obj->in_room->vnum,
              victim->hunt_obj->in_room->name);
...
/*    if ( IS_NPC(victim) && victim->hunting != NULL)
    ...
    {
      buf[0] = '\0';
      if ( victim->hunting ) sprintf(buf+strlen(buf), " hunting for (%s) %s",
                (IS_NPC(victim->hunting) ? "mobile" : "player"),
                NAME(victim->hunting));
      if ( victim->hunt_obj )
      {
        if ( victim->hunting && IS_SET(victim->hunt_flags, HUNT_CR) &&
             victim->hunt_obj->item_type == ITEM_CORPSE_PC )
          strcat(buf, " to return a corpse");
        else
          sprintf(buf+strlen(buf), " looking for (object) %s",
                  victim->hunt_obj->short_descr);
      }
      if ( IS_NPC(victim) && IS_SET(victim->hunt_flags, HUNT_MERC|HUNT_CR) &&
           victim->hunt_for )
...
void do_ofindlev( CHAR_DATA *ch, char *argument )
{
	...
	if ( ( pObjIndex = get_obj_index( vnum ) ) != NULL )
	{
        ...
        {
          sprintf( buf, "\n\r({mREMORT{x) [%3d] [%5d] %s", pObjIndex->level,
          pObjIndex->vnum, capitalize( pObjIndex->short_descr ) );
          safe_strcat(MAX_STRING_LENGTH, buf1, buf);
        }
        else
        {
          sprintf( buf, "\n\r({CMORTAL{x) [%3d] [%5d] %s", pObjIndex->level,
		      pObjIndex->vnum, capitalize( pObjIndex->short_descr ) );
          safe_strcat(MAX_STRING_LENGTH, buf1, buf);
        }
	    }
...
void do_mwhere( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( arg, "int" ) )
    {
	  ...
	  if ( IS_NPC(victim)
	  &&   victim->in_room != NULL
	  &&   IS_SET( victim->act, ACT_INTELLIGENT ) )
	  {
...
void do_shutdown( CHAR_DATA *ch, char *argument )
{
	...
	for (d = first_desc; d != NULL; d = d->next)
	{
		if(d->connected != CON_PLAYING)
			continue;
		d->character->pcdata->laston = current_time;
		save_char_obj(d->character);
	}
	save_clans();
      
    sprintf( buf, "Shutdown by %s.", ch->name );
    append_file( ch, SHUTDOWN_FILE2, buf );
    strcat( buf, "\n\r" );
    do_echo( ch, buf );
    merc_down = TRUE;
    return;
 ...
 */
	/* Loads at level it was made by default
	 */
	if ( arg2[0] == '\0' )
    ...
    {
	act( "$n {mgestures majestically, and{x $p {mappears with a crash of {WTHUNDER!{x", ch, obj, NULL, TO_ROOM );
	obj_to_char( obj, ch );
    }
    else
    {
	obj_to_room( obj, ch->in_room );
	act( "$n {mgestures, and a {x$p{m appears with a thunderous crash{x!", ch, obj, NULL, TO_ROOM );
    }
    send_to_char( "Ok.\n\r", ch );
    return;
}
...
void do_advance( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' || arg2[0] == '\0' )
    {
	send_to_char( "Syntax: Advance <player> <level>\n\r", ch );
	return;
    }
    if ( (victim = get_char_world( ch, arg1 ) ) == NULL )
     {
    ...
    level = atoi(arg2);
    victim->level = level;
    send_to_char("**** OH YESSSSS ****\n\r", victim);
    sprintf(buf2, "You are now level %d\n\r", level);
    send_to_char(buf2, victim);

    sprintf(buf2, "%s advanced to level %d.", victim->name, level);
    ...
    if ( !str_cmp( arg, "all" ) )
    {
	  ...
	  if ( !IS_NPC( vch ) )
	  {
	     if ( IS_IMMORTAL( vch ) && ( vch != ch ) )
             {
                act( "Everyone has been restored by $n." 
,ch,NULL,vch,TO_VICT);
	     }
	     else
	     {
...
void do_mset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "  str int wis dex con sex class hometown level\n\r",      ch );
	send_to_char( "  gold hp mana move practice align train\n\r",         ch );
   ...
   if ( !str_cmp( arg2, "intel") )
   {
    ...
    if ( !str_cmp( arg2, "hometown" ) )
    {
      if ( value < 0 || value >= MAX_HOMETOWN )
      {
	      char buf[MAX_STRING_LENGTH];

	      sprintf( buf, "Hometown range is 0 to %d.\n", MAX_HOMETOWN-1 );
	      send_to_char( buf, ch );
	      return;
      }
    ...
    if ( !str_cmp( arg2, "race" ) )
    {
      ...
      if (  ( !IS_NPC( victim ) )
         && ( race_table[value].player_allowed == FALSE )  )
      {
...
/*           if ( ! make_hunt(victim,hunted) )*/
	...
	{
	    send_to_char( "Hp range is -10 to 999,999 hit points.\n\r", ch );
	    return;
	}
	...
	if ( value < 0 || value > 100 )
	{
	    send_to_char( "Practice range is 0 to 100 trains.\n\r", ch );
	    return;
	}
	victim->train = value;
	return;
...
void do_users( CHAR_DATA *ch, char *argument )
{
	...
	if ( d->character != NULL )
	{
	    ...
	    switch ( d->connected )
	    {
		  ...
		  sprintf( buf3, "%s", "Get New Sex     "); break;
	       case CON_GET_NEW_CLASS:       
		  sprintf( buf3, "%s", "Con Fix Me      "); break;
	       case CON_GET_RACE:            
		  sprintf( buf3, "%s", "Get New Race    "); break;
	       case CON_READ_MOTD:           
...
void do_fights( CHAR_DATA *ch, char *argument )
{
   ...
   for ( vch = first_char; vch != NULL; vch = vch->next )
   {
	 ...
	 sprintf( buf, "%s Vs. %s  [Room:%5d]\n\r", vch->name, IS_NPC( vch->fighting ) ? 
	    vch->fighting->short_descr : vch->fighting->name,
            vch->in_room->vnum );
	 strcat( buf2, buf );
      }
   }
   ...
   for ( lch = first_char; lch; lch = lch->next )
   {
     ...
     if ( lch->hunting )
       sprintf(buf+strlen(buf), " hunting for (%s) %s",
               (IS_NPC(lch->hunting) ? "mobile" : "player"),
               NAME(lch->hunting));
     if ( lch->hunt_obj )
     {
       if ( lch->hunting && IS_SET(lch->hunt_flags, HUNT_CR) &&
            lch->hunt_obj->item_type == ITEM_CORPSE_PC )
         strcat(buf, " to return a corpse");
       else
         sprintf(buf+strlen(buf), " looking for (object) %s",
                 lch->hunt_obj->short_descr);
     }
     if ( IS_NPC(lch) && IS_SET(lch->hunt_flags, HUNT_MERC|HUNT_CR) &&
          lch->hunt_for )
...
void do_monitor( CHAR_DATA *ch, char *argument )
{
   ...
   if ( argument[0] == '\0' )
   {
      send_to_char( "{YMonitor Channel Details:{w\n\r\n\r", ch );
      for ( a = 0; monitor_table[a].min_level != 0; a++ )
      {
       ...
       send_to_char( "\n\r{YMONITOR <name> toggles the monitor channels.{w\n\r", ch );
       return;
   }
...
void monitor_chan( const char *message, int channel )
{
   ...
   if (  ( area_resetting_global )
      && ( channel == MONITOR_MAGIC ) )
     return;
   for ( a = 0; monitor_table[a].min_level != 0; a++ )
...
void do_reward( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "{xYou have been rewarded {Y%3d {CQuest Points{x by {m %s {x!!!\n\r", value, ch->name );
    send_to_char( buf, victim );
    sprintf( buf, "{xYou have rewarded {G%s  {Y%3d {CQuest Points{x!!!\n\r", victim->name, value );
    send_to_char( buf, ch );
   ...
   if ( !str_cmp ( arg2, "stop" )  )
   {
      if (  victim->hunting != NULL  )
      {
         sprintf ( buf, "%s stops hunting %s.\n\r",
                   victim->short_descr, victim->hunting->short_descr );
         end_hunt ( victim );
         send_to_char ( buf, ch );
         return;
      }
...
void do_otype( CHAR_DATA *ch, char *argument )
{
    ...
    */
    for ( vnum = 0; nMatch < top_obj_index; vnum++ )
    {
        ...
        {
            nMatch++;
                if ( fAll || is_name( arg, tab_item_types[
(pObjIndex->item_type)-1 ].text ) )
            {
                found = TRUE;
...
bit_table_lookup(tab_wear_flags, pObjIndex->wear_flags) ) )
            {
                ...
                sprintf( buf, "<%s> [%5d] [%3d] %s %s\n\r",
                  bit_table_lookup(tab_wear_flags, pObjIndex->wear_flags),
                   ...
                   ( IS_SET( pObjIndex->extra_flags, ITEM_REMORT ) ?
                     "{mRemort{x" :
                     "{CMortal{x" ),  pObjIndex->short_descr );
                safe_strcat(MAX_STRING_LENGTH, buf1, buf);
            }
        }
    }
...
bool are_enemies( CLAN_DATA *pclan, CLAN_DATA *search)
{
	char pclan_numb[MSL];
	char search_numb[MSL];
	char pclan_enemies[MSL];
	char search_enemies[MSL];
		sprintf(pclan_numb, "%d", pclan->clan_numb);
		sprintf(search_numb, "%d", search->clan_numb);
		sprintf(pclan_enemies, "%s", pclan->enemies);
		sprintf(search_enemies, "%s", search->enemies);
        if ( !num_infix( search_numb, pclan_enemies) 
			&& !num_infix( pclan_numb, search_enemies) )
			return TRUE;
		return FALSE;
}
int get_clan_numb()
{
	...
	for( cnum = first_clan; cnum != NULL; cnum = cnum->next )
	{
	bufz[0] = '\0';
    sprintf(bufz, "%d ", cnum->clan_numb);
	strcat(buf, bufz);
    gotone = TRUE;
	}
   /* It's the first clan on the list...*/
	if (!gotone)
		return 1;
	/* Now we go though and check for the first lowest number that's assigned
	 * to another clan.
	 */
	for( num = 1; num < MAX_CLAN; num++)
	{
		comp[0] = '\0';
		sprintf(comp, "%d", num);
		
		if(num_infix(comp, buf) )
			return num;
	}
		return -1;
}
void do_addclan( CHAR_DATA *ch, char *argument )
{
	...
	CLAN_DATA *search;
	CHAR_DATA *victim;
	argument = one_argument_st( argument, arg1 );
	argument = one_argument_st( argument, arg2 );
	argument = one_argument_st( argument, arg3 );

	if (IS_NPC(ch) )
		return;
	if ( arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0' )
	{
		send_to_char("Syntax: addclan <clan name> <clan abbriv> <clan leader>\n\r", ch );
		return;
	}
	for (search = first_clan; search != NULL; search = search->next )
	{
		if (!str_cmp(search->clan_name, arg1) )
		{
			send_to_char("There is already a clan with that name!\n\r", ch);
			return;
		}
		if (!str_cmp(search->clan_abbr, arg2) )
		{
			send_to_char("There is already a clan with that abbr!\n\r", ch);
			return;
		}
		if (!str_cmp(search->leader, arg3) )
		{
			send_to_char("That person is already the leader of another clan!\n\r", ch);
			return;
		}
	...
	pClan->clan_name = str_dup(arg1);
	pClan->clan_abbr = str_dup(arg2);
	pClan->clan_room = 0;
	pClan->donat_room = 0;
	pClan->enemies =   str_dup("");
	pClan->members =   str_dup(victim->name);
	pClan->leader =    str_dup(victim->name);
	for ( ceq = 0; ceq < MAX_CLAN_EQ; ceq++ );
	pClan->eq[ceq] = 0;
	pClan->clan_numb = get_clan_numb(ch);
    LINK(pClan, first_clan, last_clan, next, prev);
	victim->pcdata->clan = pClan->clan_numb;
    SET_BIT(victim->pcdata->pflags, PFLAG_CLAN_LEADER );
	SET_BIT(victim->pcdata->pflags, PFLAG_CLAN_BOSS );
	save_clans();
	send_to_char("Clan added.\n\r", ch);
	if (victim != ch)
	send_to_char("A clan has been made and you are it's leader.\n\r", victim);
	return;
}
...
void do_delclan( CHAR_DATA *ch, char *argument )
{
	...
	if (argument[0] == '\0')
	{
		send_to_char("Delete which clan number?\n\r", ch);
		return;
	}

	if (!is_number(argument) )
	{
		send_to_char("Syntax: delclan <{Rclan number{x>\n\r", ch);
		return;
	}


	for( clan = first_clan; clan != NULL; clan = clan->next )
		if (clan->clan_numb == atoi(argument) )
		{
			for (ppl = first_char; ppl != NULL; ppl = ppl->next)
			{
				if(IS_NPC(ppl))
					continue;
				
				if (ppl->pcdata->clan == 0)
					continue;  // Just in case...

			    if (clan->clan_numb != ppl->pcdata->clan)
					continue;

			ppl->pcdata->clan = 0;
			REMOVE_BIT(ppl->pcdata->pflags, PFLAG_CLAN_LEADER );
	        REMOVE_BIT(ppl->pcdata->pflags, PFLAG_CLAN_DIPLOMAT );
	        REMOVE_BIT(ppl->pcdata->pflags, PFLAG_CLAN_BOSS );
	        REMOVE_BIT(ppl->pcdata->pflags, PFLAG_CLAN_TREASURER );
	        REMOVE_BIT(ppl->pcdata->pflags, PFLAG_CLAN_ARMOURER );

			if(ppl != ch)
				send_to_char("Your clan has been deleted by an Imm!\n\rAll clan flags removed!\n\r", ppl);
			}
        for (search = first_clan; search != NULL; search = search->next)
			 if (are_enemies(clan, search) )
			 {
				sprintf(clan_numb, "%d", clan->clan_numb);
         		sprintf(search_numb, "%d", search->clan_numb);
		        sprintf(clan_enemies, "%s", clan->enemies);
		        sprintf(search_enemies, "%s", search->enemies);

		
			    sprintf(modbuf, "-%d", search->clan_numb);
		      	clan->enemies = num_mod(clan->enemies, modbuf);
			    fix_enemies(clan);

			    sprintf(modbuf, "-%d", clan->clan_numb);
			    search->enemies = num_mod(search->enemies, modbuf);
			    fix_enemies(search);
			 }
	...
	send_to_char("No such clan number found!\n\r", ch);
	return;
}
void do_setclan( CHAR_DATA *ch, char *argument )
{
	...
	if (IS_NPC(ch) )
		return;
	if (  arg1[0] == '\0' || !is_number(arg1) || arg2[0] == '\0' || arg3[0] == '\0' )
	{

		send_to_char("Syntax: setclan <{Rclan number{x> <variable> <argument>\n\r\n\r", ch );
		send_to_char("Variables are:\n\r", ch);
		send_to_char("Name Abbr ClanRoom\n\r", ch);
		send_to_char("Donation(Donate room) Enemies(Enemy clans)\n\r", ch);
		send_to_char("Owner(Clan OWNER)\n\r", ch);
		send_to_char("Eq1-6(ie: Eq1, Eq5)\n\r", ch); 
		return;
	}
	cn = atoi(arg1);
	...
	if (!found)
	{
		send_to_char("No such clan in the clan table!\n\r", ch);
		send_to_char("Type 'showclan' to see a list of clans.\n\r", ch);
		return;
	}
	...
	if (!str_cmp(arg2, "Name") )
	{
		for( search = first_clan; search != NULL; search = search->next )
		{
			...
			if (!str_cmp(search->clan_name, arg3) )
			{
				send_to_char("There is already a clan with that name!\n\r", ch);
				return;
			}
			if (!str_prefix(arg3, search->clan_name) )
			{
				send_to_char("Clan name too similer to another clan already.\n\r", ch);
				return;
			}
		}
		free_string(pclan->clan_name);
		pclan->clan_name = str_dup(arg3);
		send_to_char("Clan name changed.\n\r", ch);

		if ( ( chl = get_char_world( ch, pclan->leader ) ) != NULL )
			if (chl != ch)
			send_to_char("Your clan name has been changed.\n\r", chl);
		save_clans();
		return;
	}

     if (!str_cmp(arg2, "Abbr") )
	{
		for( search = first_clan; search != NULL; search = search->next )
		{
				if (search == pclan)
				continue;
			if (!str_cmp(search->clan_abbr, arg3) )
			{
				send_to_char("There is already a clan with that abbr!\n\r", ch);
				return;
			}
			if (!str_prefix(arg3, search->clan_name) )
			{
				send_to_char("Clan abbr too similer to another clan already.\n\r", ch);
				return;
			}
		}
		free_string(pclan->clan_abbr);
		pclan->clan_abbr = str_dup(arg3);
		send_to_char("Clan abbr changed.\n\r", ch);

		if ( ( chl = get_char_world( ch, pclan->leader ) ) != NULL )
			if (chl != ch)
			send_to_char("Your clan abbr has been changed.\n\r", chl);
		save_clans();
		return;
	}

	if (!str_cmp(arg2, "ClanRoom") )
	{
		if(!is_number(arg3) )
		{
			send_to_char("ClanRoom must be room vnum!\n\r", ch);
			return;
		}
		clanroom = atoi( arg3 );
		if (get_room_index(clanroom) == NULL && clanroom != 0 )
		{
		...
		for( search = first_clan; search != NULL; search = search->next )
		{
				if (search == pclan)
				continue;
			if ( search->clan_room == clanroom)
			{
				send_to_char("There is already a clan with that room as a clan room.\n\r", ch);
				return;
			}
		}

		pclan->clan_room = clanroom;
		send_to_char("Clan room changed.\n\r", ch);

		if ( ( chl = get_char_world( ch, pclan->leader ) ) != NULL )
			if (chl != ch)
			send_to_char("Your clan ROOM has been changed.\n\r", chl);
		save_clans();
		return;
	}
	...
	if (!str_cmp(arg2, "Donation") )
	{
		if(!is_number(arg3) )
		{
			send_to_char("Donation must be room vnum!\n\r", ch);
			return;
		}
		clanroom = atoi( arg3 );
		if (get_room_index(clanroom) == NULL && clanroom != 0 )
		...
		for( search = first_clan; search != NULL; search = search->next )
		{
				if (search == pclan)
				continue;
			if ( search->donat_room == clanroom)
			{
				send_to_char("There is already a clan with that room as a donation room.\n\r", ch);
				return;
			}
		}

		pclan->donat_room = clanroom;
		send_to_char("Donation room changed.\n\r", ch);

		if ( ( chl = get_char_world( ch, pclan->leader ) ) != NULL )
			if (chl != ch)
			send_to_char("Your DONATION ROOM has been changed.\n\r", chl);
		save_clans();
		return;
	}
	...
	if (!str_cmp(arg2, "Enemies") )
	{
		if (!is_number(arg3) )
		{
			send_to_char("Which CLAN number would you like to add/remove to the list?\n\r", ch);
			return;
		}
		for( search = first_clan; search != NULL; search = search->next )
		{
        ...
        if (!eclan)
		{
			send_to_char("No such clan to make/clear enemies with!\n\r", ch);
			return;
		}
		sprintf(pclan_numb, "%d", pclan->clan_numb);
		sprintf(search_numb, "%d", search->clan_numb);
		...
		{
			sprintf(modbuf, "-%d", search->clan_numb);
			pclan->enemies = num_mod(pclan->enemies, modbuf);
			fix_enemies(pclan);

			sprintf(modbuf, "-%d", pclan->clan_numb);
			search->enemies = num_mod(search->enemies, modbuf);
			fix_enemies(search);

			if ( ( chl = get_char_world( ch, pclan->leader ) ) != NULL )
			{
				sprintf(buf, "You are no longer at war with %s.\n\r", search->clan_name);
				if (chl != ch)
			    send_to_char(buf, chl);
			}
			if ( ( che = get_char_world( ch, search->leader ) ) != NULL )
			{
				sprintf(buf, "You are no longer at war with %s.\n\r", pclan->clan_name);
				if (che != ch)
			    send_to_char(buf, che);
			}
		} else
		{
			if ( num_infix( search_numb, pclan_enemies) 
			&& num_infix( pclan_numb, search_enemies) )
			{
				sprintf(modbuf, "%s ", pclan_enemies);
				strcat(modbuf, search_numb);
				free_string(pclan->enemies);
				pclan->enemies = str_dup(modbuf);

				sprintf(modbuf, "%s ", search_enemies);
				strcat(modbuf, pclan_numb);
				free_string(search->enemies);
				search->enemies = str_dup(modbuf);

			    if ( ( chl = get_char_world( ch, pclan->leader ) ) != NULL )
				{
				sprintf(buf, "You are {RAT WAR{x with %s.\n\r", search->clan_name);
				if (chl != ch)
			    send_to_char(buf, chl);
				}
			    if ( ( che = get_char_world( ch, search->leader ) ) != NULL )
		...
		save_clans();
	        ...
	        pclan_enemies[0] = '\0';
	        search_enemies[0] = '\0';
			send_to_char("Enemies set for BOTH clans.\n\r", ch);
		return;
	} /*Enemies */

	if (!str_cmp(arg2, "Owner") )
	{
		if ( ( chn = get_char_world(ch, arg3) ) == NULL)
		{
			send_to_char("That person isn't logged on!\n\r", ch);
			return;
		}
		if (IS_NPC(chn) )
		{
			send_to_char("NPCs can't be clan owners!\n\r", ch);
			return;
		}
		...
		for( search = first_clan; search != NULL; search = search->next )
		{
			if (search == pclan)
				continue;
			if (!str_cmp(search->leader, chn->name) )
			{
				send_to_char("That person is already the owner of another clan!\n\r", ch);
				return;
			}
		}
		if (chn->pcdata->clan != pclan->clan_numb)
		{
			send_to_char("That person doesn't belong to the clan in question.\n\r", ch);
			return;
		}
		if ( ( chl = get_char_world(ch, pclan->leader) ) != NULL )
		{
			send_to_char("You are no longer the OWENER of your clan.\n\r", chl);
			REMOVE_BIT(chl->pcdata->pflags, PFLAG_CLAN_BOSS );
		}
		/* Not the owner of the clan but still a leader
		 */
		free_string(pclan->leader);
		pclan->leader = str_dup(chn->name);
		SET_BIT(chn->pcdata->pflags, PFLAG_CLAN_LEADER );
		SET_BIT(chn->pcdata->pflags, PFLAG_CLAN_BOSS );
		send_to_char("Clan owner changed.\n\r", ch);
		if (ch != chn)
		send_to_char("You are now CLAN OWNER.\n\r", chn);
	...
	if (!str_cmp(arg2, "Eq1") )
	{
		if(!is_number(arg3) )
		{
			send_to_char("Clan eq has to be vnum of obj!\n\r", ch);
			return;
		}
		ceq = atoi( arg3 );
		if (get_obj_index(ceq) == NULL && ceq != 0)
		{
			send_to_char("Invalid vnum!\n\r", ch);
			return;
		}
		pclan->eq[0] = ceq;
		send_to_char("Clan eq 1 set.\n\r", ch);

		if ( ( chl = get_char_world( ch, pclan->leader ) ) != NULL )
			if( chl != ch)
			send_to_char("Your Clan eq 1 has been changed.\n\r", chl);
		save_clans();
		return;
	}
		if (!str_cmp(arg2, "Eq2") )
	{
		...
		pclan->eq[1] = ceq;
		send_to_char("Clan eq 2 set.\n\r", ch);

		if ( ( chl = get_char_world( ch, pclan->leader ) ) != NULL )
			if( chl != ch)
			send_to_char("Your Clan eq 2 has been changed.\n\r", chl);
		save_clans();
		return;
	}
	if (!str_cmp(arg2, "Eq3") )
	{
		...
		pclan->eq[2] = ceq;
		send_to_char("Clan eq 3 set.\n\r", ch);

		if ( ( chl = get_char_world( ch, pclan->leader ) ) != NULL )
			if( chl != ch)
			send_to_char("Your Clan eq 3 has been changed.\n\r", chl);
		save_clans();
		return;
	}
	if (!str_cmp(arg2, "Eq4") )
	{
		...
		pclan->eq[3] = ceq;
		send_to_char("Clan eq 4 set.\n\r", ch);

		if ( ( chl = get_char_world( ch, pclan->leader ) ) != NULL )
			if( chl != ch)
			send_to_char("Your Clan eq 4 has been changed.\n\r", chl);
		save_clans();
		return;
	}
	if (!str_cmp(arg2, "Eq5") )
	{
		...
		pclan->eq[4] = ceq;
		send_to_char("Clan eq 5 set.\n\r", ch);

		if ( ( chl = get_char_world( ch, pclan->leader ) ) != NULL )
			if( chl != ch)
			send_to_char("Your Clan eq 5 has been changed.\n\r", chl);
		save_clans();
		return;
	}
	if (!str_cmp(arg2, "Eq6") )
	{
		...
		pclan->eq[5] = ceq;
		send_to_char("Clan eq 6 set.\n\r", ch);

		if ( ( chl = get_char_world( ch, pclan->leader ) ) != NULL )
			if( chl != ch)
			send_to_char("Your Clan eq 6 has been changed.\n\r", chl);
		save_clans();
		return;
	}
...
void do_showclan( CHAR_DATA *ch, char *argument)
{
	...
	if (argument[0] == '\0')
	{
		sender[0] = '\0';
		buf[0] = '\0';
	for( cn = 1; cn < MAX_CLAN; cn++)
	  for( clan = first_clan; clan != NULL; clan = clan->next )
	  {
		/* To make sure that the showclan list is always in order, even if
		 * the file order isn't.
		 */
		if(cn != clan->clan_numb)
			continue;
		sprintf(buf, "Clan %d:\n\r", clan->clan_numb );
		strcat(sender, buf);
		sprintf(buf, "  Name: %s\n\r", clan->clan_name);
	...
	} else
	{
		...
		if (!is_number(argument) )
		{
			send_to_char("Syntax: showclan <clannumb>\n\r OR \n\rSyntax: showclan\n\r", ch);
			return;
		}
		for (clan = first_clan; clan != NULL; clan = clan->next)
		{
			if (atoi(argument) == clan->clan_numb)
			{
				gotone = TRUE;
				break;
			}
		...
		if (!gotone)
		{
			send_to_char("There is no such clan by that number.\n\rType 'showclan' to see all clans.\n\r", ch);
			return;
		}
		room = get_room_index(clan->clan_room);
		dona = get_room_index(clan->donat_room);
		sprintf(buf, "Clan %d:\n\r", clan->clan_numb );
		strcat(sender, buf);
		sprintf(buf, "  Name: %s\n\r", clan->clan_name);
		strcat(sender, buf);
		...
		if (IS_IMMORTAL(ch) || ch->pcdata->clan == clan->clan_numb)
		{
			sprintf(buf, "  Donation: %s (%d)\n\r",
			(clan->donat_room == 0 ? "Not set!" : dona->name ),
			(clan->donat_room == 0 ? 0 : dona->vnum ) );
		strcat(sender, buf);
			sprintf(buf, "  Clan Room: %s (%d)\n\r",
			(clan->clan_room == 0 ? "Not Set!" : room->name ),
			(clan->clan_room == 0 ? 0 : room->vnum ) );
			strcat(sender, buf);
		}
		sprintf(buf, "  Owner: %s\n\r", clan->leader);
		strcat(sender, buf);
		sprintf(buf, "  Enemies:");
		strcat(sender, buf);
		for (enemies = first_clan; enemies != NULL; enemies = enemies->next)
		{
			sprintf(en, "%d", enemies->clan_numb);
			if (!num_infix(en, clan->enemies) )
			{
				if (!fn)
					strcat(sender, "\n\r");
				sprintf(buf, "    %s\n\r", enemies->clan_name);
		...
		if (IS_IMMORTAL(ch) || ch->pcdata->clan == clan->clan_numb)
		{
			...
			for( nums = 0; nums < MAX_CLAN_EQ; nums++)
			{
			 if ( (ceq = get_obj_index(clan->eq[nums]) ) == NULL )
				continue;
			  found = TRUE;
			  sprintf(buf, "    Eq %d: %-4d {r-{d={r-{x %s",( nums + 1), ceq->vnum, ceq->short_descr);
			  strcat(sender, buf);
			  strcat(sender, "\n\r");
			}
			if (!found)
			{
				strcat(sender, "    This clan has no clan eq!\n\r");
				sprintf(buf, "  Members: %s", clan->members);
		        strcat(sender, buf);
				strcat(sender, "\n\r");
	...
	send_to_char(sender, ch);
	if (argument[0] == '\0')
	send_to_char("Type 'showclan <clan number>' to see more information about a clan.\n\r",ch);
	return;
}

void raw_delclan( CHAR_DATA *ch, char *argument )
{
	CLAN_DATA *clan;
	CLAN_DATA *search;
	CHAR_DATA *ppl;
	...
	for( clan = first_clan; clan != NULL; clan = clan->next )
		if (clan->clan_numb == atoi(argument) )
		{
			...
			if(ppl != ch)
				send_to_char("Your clan has been deleted because the owner was purged/deleted!\n\rAll clan flags removed!\n\r", ppl);
			}
        for (search = first_clan; search != NULL; search = search->next)
			 if (are_enemies(clan, search) )
			 {

...
bool is_name( const char *str, char *namelist )
{
	...
	if ( name[0] == '\0' )
	    return FALSE;
	if ( !str_cmp_name( str, name ) )
	    return TRUE;
    }
}

bool is_name_note( const char *str, char *namelist )
{
    char name[MAX_INPUT_LENGTH];
...
bool str_cmp_name( const char *astr, const char *bstr )
{
  int count = 0;
  bool notrunc = FALSE;
    if ( astr == NULL )
...
/*	bug( "Str_cmp: null bstr.", 0 );  */
  ...
  for ( ; *astr || *bstr; astr++, bstr++ )
  {
    if( !notrunc ) /* Truncating allowed, match partials and *'s */
    {
     if( *astr == '*' && count )
       return FALSE;
     else if( (*astr == '\0' && *bstr != '\0') && count )
       return FALSE;
    }
...
bool num_infix( const char *astr, const char *bstr )
{
    int sstr1;
    int sstr2;
    int ichar;
    ...
    for ( ichar = 0; ichar <= sstr2; ichar++ )
    {
	  if ( c0 == LOWER(bstr[ichar]) && !str_prefix( astr, bstr + ichar )  )
	  {
		if (ichar + 1 != sstr2  && isspace(bstr[ichar+1]) )
	    return FALSE;
		else if (ichar + 1 == sstr2)
		return FALSE;
	  }
    }
...

{
  ...
  switch ( ch->adept_level )
  {
    ...
    case 1: return    "{W    Mystic    {x";
            break;
    case 2: return    "{C   Templar    {x";
            break;
    case 3: return    "{B Illusionist  {x";
            break;
    case 4: return    "{R   Crusader   {x";
            break;
    case 5: return    "{d   Warlock    {x";
            break;
    case 6: return    "{C   Paladin    {x";

    case 7: return    "{G    Ranger    {x";
            break;
    case 8: return    "{c  Gladiator   {x";
            break;
    case 9: return    "{B    Shogun    {x";
            break;
    case 10: return   "{R    Shamen    {x";
            break;
    case 11: return   "{G    Druid     {x";
            break;
    case 12: return   "{y  Conjurer    {x";

    case 13: return   "{B Elementalist {x";
            break;
    case 14: return   "{m  Runemaster  {x";

    case 15: return   "{d Shadowmaster {x";
            break;
    case 16: return   "{y Beastmaster  {x";
            break;
    case 17: return   "{r   Warlord    {x";
            break;
    case 18: return   "{R  Dragonlord  {x";
            break;
    case 19: return   "{d  Demonlord   {x";
            break;
    case 20: return   "{m  Realm Lord  {x";
...
char * num_mod( char * mod_string,  char *argument )
{
    ...
    if ( argument[0] == '+' || argument[0] == '-' )
    {
      ...
      if ( argument[0] == '+' )
      {
        ...
        while ( isspace( *argument ) )
          argument++;
        if ( !num_infix( argument, mod_string ) )
        {
          return mod_string;
        }
        strcat( buf, argument );
...
char *quote( char *argument )
{
    static char rquote[MSL];
	int i = 0;
	int cnt = 0;

	if ( argument == NULL )
      return NULL;
    while ( isspace(*argument) && strlen(argument) <= cnt )
	{
		argument++;
		cnt++;
	}
...
char *read_line( char *argument, char *output )
{
    if ( argument == NULL )
      return NULL;
    while ( isspace(*argument) )
...
char *one_argument_st( char *argument, char *arg_first )
{
    char cEnd;
    if ( argument == NULL )
      return NULL;
...
char *get_tribe_standing_name( int standing )
{
  ...
  switch ( standing )
  {
    case 0: return    "{mEternal{x";
            break;
    case 1: return    "{GEldest{x";
            break;
    case 2: return    "{GMate{x";
            break;
    case 3: return    "{RWarder{x";
            break;
    case 4: return    "{gGuardian{x";
            break;
    case 5: return    "{CSentry{x";
            break;
    case 6: return    "{YElder{x";

    case 7: return    "{GAdult{x";
            break;
    case 8: return    "{rYounger{x";
            break;
    case 9: return    "{yCub{x";
            break;
...
char *get_moon_phase_name( void )
{
  ...
  {
    case MOON_NEW     : return    "Dark";
                        break;
    case MOON_WAX_CRE : return    "{dCresent{x";
                        break;
    case MOON_WAX_HALF: return    "{wHalf{x";
                        break;
    case MOON_WAX_GIB : return    "{WGibbous{x";
                        break;
    case MOON_FULL    : return    "{WFULL{x";
                        break;
    case MOON_WAN_GIB : return    "{WGibbous{x";
                        break;
    case MOON_WAN_HALF: return    "{wHalf{x";
                        break;
    case MOON_WAN_CRE : return    "{dCrescent{x";
                        break;
...
char *raffect_bit_name( int vector )
{
  ...
  rbuf[0] = '\0';
    ...
    if ( vector & ROOM_BV_HEAL_REGEN	) safe_strcat( MAX_STRING_LENGTH,  rbuf, " {mHealing Light{x" );
    if ( vector & ROOM_BV_HEAL_STEAL	) safe_strcat( MAX_STRING_LENGTH,  rbuf, " {dWither Shadow{x"          );
    if ( vector & ROOM_BV_MANA_REGEN	) safe_strcat( MAX_STRING_LENGTH,  rbuf, " {RMana Flare{x"     );
    if ( vector & ROOM_BV_MANA_STEAL	) safe_strcat( MAX_STRING_LENGTH,  rbuf, " {dMana Drain{x"   );
    if ( vector & ROOM_BV_FIRE_RUNE	) safe_strcat( MAX_STRING_LENGTH,  rbuf, " {RFire {xRune"      );
    if ( vector & ROOM_BV_FIRE_TRAP	) safe_strcat( MAX_STRING_LENGTH,  rbuf, " {RFire {xTrap"      );
    if ( vector & ROOM_BV_DAMAGE_TRAP	) safe_strcat( MAX_STRING_LENGTH,  rbuf, " {dDamage {xTrap"       );
    if ( vector & ROOM_BV_SHOCK_RUNE	) safe_strcat( MAX_STRING_LENGTH,  rbuf, " {BShock {xRune"     );
    if ( vector & ROOM_BV_SHOCK_TRAP	) safe_strcat( MAX_STRING_LENGTH,  rbuf, " {BShock {xTrap"     );
    if ( vector & ROOM_BV_HOLD     	) safe_strcat( MAX_STRING_LENGTH,  rbuf, " {GCage{x"         );
    if ( vector & ROOM_BV_POISON_RUNE   ) safe_strcat( MAX_STRING_LENGTH,  rbuf, " {mPoison{x Rune" );
...
char *  get_family_name     ( CHAR_DATA *ch ) 
{
	...
	case 1  : return "{WNOT SET{x" ;
	case 2  : return "{WNOT SET{x" ;
	case 3  : return "{WNOT SET{x" ;
	case 4  : return "{WNOT SET{x";
      }

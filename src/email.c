  ...
  if ( !str_cmp( arg1, "set" ) )
  {
    ...
    if ( valid_email )
    {
      ...
      send_to_char( outbuf, ch );
      {
        ...
        for ( brand_member = first_brand;
              brand_member;
              brand_member = brand_member->next )
        {
        ...
        sprintf( brandbuf, "Email address validation request for %s, address %s\n\rPlease type email validate %s to authorize.\n\r",
             ch->name,
             ch->pcdata->email_address,
             ch->name );
...
void send_rep_out( CHAR_DATA * ch, char * outbuf, bool mailme, char * msub )
{
  ...
  if ( mailme )
  {
       ...
       && (  str_cmp( ch->pcdata->email_address, "not set" ) )  )
    {
      ...
      {
        send_to_char( outbuf, ch );
        send_to_char( "\n\r{RUNABLE TO SEND SYSTEM MAIL. {WCheck your sendmail settings.{x\n\r", ch );
      }
    }
    else
    {

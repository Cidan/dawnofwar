...
void objfun_soul_moan( OBJ_DATA *obj, CHAR_DATA *keeper )
{
   if ( keeper == NULL || keeper->in_room == NULL  )
     ...
     if ( number_percent() < 2 )
     {
        act( "{xThe {RSoul{x in $p{x carried by {C$n{x moans in agony.", keeper, obj, NULL, TO_ROOM );
        act( "{xThe {RSoul{x in $p {xcarried by you moans to be set free!", keeper, obj, NULL, TO_CHAR );
        return;
     }

     if ( number_percent() < 2 )
     {
        act( "{xThe {RSoul{x in $p{x carried by {C$n{x tries to break free of it's inprisonment!", keeper, obj, NULL, TO_ROOM );
        act( "{xThe {RSoul{x in $p {xcarried by you starts writhing--look out!!", keeper, obj, NULL, TO_CHAR );
        return;
     }
...
void objfun_cast_fight( OBJ_DATA *obj, CHAR_DATA *keeper )
{
   ...
   switch ( number_range( 0, 5 ) )
   {
      ...
      case 1: min_lev =  7; spell = "color spray";		break;
...
void objfun_flaming( OBJ_DATA *obj, CHAR_DATA *keeper )
{
   ...
   if ( victim == NULL )
   {
     if ( number_percent() < 2 )
     {
        act( "{x$p{x carried by {C$n{R flames {Ybrightly{x!", keeper, obj, NULL, TO_ROOM );
        act( "{x$p {xcarried by you{R flames {Ybrightly{x!", keeper, obj, NULL, TO_CHAR );
        return;
     }

     if ( number_percent() < 2 )
     {
        act( "{x$p{x carried by {C$n{x goes {ddim{x.", keeper, obj, NULL, TO_ROOM );
        act( "{x$p {xcarried by you{x goes {ddim{x.", keeper, obj, NULL, TO_CHAR );
        return;
     }
     return;
   }
...
void objfun_healing( OBJ_DATA *obj, CHAR_DATA *keeper )
{
   ...
   if ( victim == NULL )
   {
     if ( number_percent() < 2 )
     {
        act( "{x$p{x carried by {C$n{Y glows{x with a {mHoly {Wlight{x!", keeper, obj, NULL, TO_ROOM );
        act( "{x$p {xcarried by you{Y glows{x with a {mHoly {Wlight{x!", keeper, obj, NULL, TO_CHAR );
        return;
     }

     if ( number_percent() < 2 )
     {
        act( "{x$p{x carried by {C$n{x sings a {mhymn{x.", keeper, obj, NULL, TO_ROOM );
        act( "{x$p {xcarried by you{x sings a hymn{x.", keeper, obj, NULL, TO_CHAR );
        return;
     }
     return;
   }
   ...
   if ( ( sn = skill_lookup( spell ) ) < 0 )
      return;
   act( "$p blankets $n {Win a {mhealing {Waura{x!", keeper, obj, NULL, TO_ROOM );
   act( "$p blankets you in a {mhealing {Waura{x!", keeper, obj, NULL, TO_CHAR );
...
void objfun_dispeller( OBJ_DATA *obj, CHAR_DATA *keeper )
{
   ...
   if ( victim == NULL )
   {
     if ( number_percent() < 2 )
     {
        act( "{x$p{x carried by {C$n{C vibrates {xsuddenly{x!", keeper, obj, NULL, TO_ROOM );
        act( "{x$p {xcarried by you{C vibrates {xsuddenly{x!", keeper, obj, NULL, TO_CHAR );
        return;
     }

     if ( number_percent() < 2 )
     {
        act( "{x$p{x carried by {C$n{C peers{x at your inventory{x.", keeper, obj, NULL, TO_ROOM );
        act( "{x$p {xcarried by you{C peers{x around the room{x.", keeper, obj, NULL, TO_CHAR );
        return;
     }
     return;
   }
   ...
   if ( ( sn = skill_lookup( spell ) ) < 0 )
      return;
   act( "$p {Cpeers{x deeply into the mind of $n!", victim, obj, NULL, TO_ROOM );
   act( "$p {Cpeers {xinto your thoughts!", victim, obj, NULL, TO_CHAR );
   obj_cast_spell( sn, obj->level, keeper, victim, obj );
   return;
}

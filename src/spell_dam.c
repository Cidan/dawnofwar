   ...
   case RE_FIRE:	
   {
     if ( HAS_BODY( victim ) )
     {
      ...
      case 0:
         ...
         sprintf( buf2, "You cremate $N's body to a crispy {RRED{x cinder!"   );
...
void sp_dam_message( OBJ_DATA * obj, CHAR_DATA *ch, CHAR_DATA *victim, int dam, int realm, int dt )
{
    ...
    {        
      sprintf( testerbuf, " {B({W%i{B){x", dam );
    }
    else
      sprintf( testerbuf, "%s", "" );
...
*/
  ...
  if ( obj == NULL )
  {	
	   sprintf( buf1, "%s$n %s%s%s{x $N %swith $s %s%c{x%s",
        ...
        punct,
        testerbuf );
	   sprintf( buf2, "%sYou %s%s%s{x $N %swith your %s%c{x%s",
        sp_dam_str[rtype].basecol,
        symbuf,
        ...
        testerbuf );
/*	   if ( *str == '\'' )
	    sprintf( buf3, "%s$n %s%s your%s%s $s %s%c{x",col,col, vp,col, str+2, attack, punct);
	   else
*/
	    sprintf( buf3, "%s$n %s%s%s %syou with $s %s%c{x%s",
      sp_dam_str[rtype].basecol, 
      symbuf,
      ( stress ? sp_dam_str[rtype].vp_stress : sp_dam_str[rtype].vp ),
      symbuf, 
  ...
  {	
	   sprintf( buf1, "%s$p %s%s%s{x $N %swith a %s%c{x",
        sp_dam_str[rtype].basecol,
        symbuf, 
        ( stress ? sp_dam_str[rtype].vp_stress : sp_dam_str[rtype].vp ),
        symbuf,
...
*/
	    sprintf( buf3, "%s$p %s%s%s %syou with $s %s%c{x",
      sp_dam_str[rtype].basecol, 
      symbuf,
      ( stress ? sp_dam_str[rtype].vp_stress : sp_dam_str[rtype].vp ),
      symbuf, 
 ...
 */
  ...
  if ( obj == NULL )
  {
       ...
       && ( number_percent( ) < ( get_psuedo_level( victim ) - 70 ) )  )
    {
      ...
      act( "{x$n's {Bc{Rl{Go{Ya{Ck{x glows brightly as $Nn's spell hits it, and the spell is reflected{x!!", ch, victim, NULL, TO_ROOM );
      act( "{x$N's {Bc{Rl{Go{Ya{Ck{x glows brightly, and reflects your spell back on you{x!!", ch, NULL, victim, TO_CHAR );
      act( "{xYour {Bc{Rl{Go{Ya{Ck{x glows brightly, and reflects the spell back on $N{x!!!", victim, NULL, ch, TO_CHAR );
...
/*    dam = 3000; */
  ...
  if ( victim != ch )
  {
    ...
    if (  ( show_msg ) 
       && ( dam >= 0 )  )
       dam_message( ch, victim, dam, dt, sn );

...
char *get_ruler_title( int ruler_rank, int sex )
{
  ...
  {
    case 0 :  return "";
    case 1 :  return ( ( sex == SEX_NEUTRAL ) ? "{WMonitor{x " 
                       : ( sex == SEX_MALE ) ? "{WLord{x " 
                       : "{WLady{x " );
              break;
    case 2 :  return ( ( sex == SEX_NEUTRAL ) ? "{ROverseer{x " 
                       : ( sex == SEX_MALE ) ? "{RBaron{x " 
                       : "{RBaroness{x " );
              break;
    case 3 :  return ( ( sex == SEX_NEUTRAL ) ? "{mController{x " 
                       : ( sex == SEX_MALE ) ? "{mDuke{x " 
                       : "{mDuchess{x " );
              break;
    case 4 :  return ( ( sex == SEX_NEUTRAL ) ? "{BDirector{x " 
                       : ( sex == SEX_MALE ) ? "{BPrince{x " 
                       : "{BPrincess{x " );
              break;
    case 5 :  return ( ( sex == SEX_NEUTRAL ) ? "{CDominator{x " 
                       : ( sex == SEX_MALE ) ? "{CKing{x " 
                       : "{CQueen{x " );
              break;
  }
  return "Lord";
    ...
    for ( ; ; )
    {
      ...
      if ( !str_cmp( word, "#RULER" ) ) 
      {
        ...
        if ( ruler->affiliation_index == 0 )
          ...
          ruler->affiliation_name = str_dup( get_clan_info(ruler->affiliation_index, CABBR) );
...
/*  CONTROL_DATA	* this_control;  */
  ...
  if ( arg1[0] == '\0' ) /* show current rulers */
  {
    ...
    for ( ruler = first_ruler_list; ruler; ruler = ruler->next )
    {
      ...
      sprintf( catbuf, " %s %s : %s \n\r", 
        ...
        ( IS_SET( ruler->this_one->flags, RULER_GROUP ) ?
             ruler->this_one->affiliation_name : "Not Affiliated" )  );
  ...
  if ( !str_prefix( arg1, "affiliation" ) )
  {
    ...
    if ( is_number( arg3 ) )
    {
      ...
      ruler->this_one->affiliation_name = str_dup( get_clan_info(clan_number, CABBR) );

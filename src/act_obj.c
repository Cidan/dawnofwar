...
void get_obj( CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *container )
{
    ...
    {
      send_to_char( "Your claws are too clumsy!{x\n\r", ch );
      return;
    }
...
argument = one_argument(argument, arg3);
	...
	if (arg1[0] == '\0')
	{
		send_to_char("Get what where?\n\r", ch);
		return;
	}
    /* 'get x money item' */
     if (is_number(arg1) && arg2[0] != '\0' && arg3[0] != '\0')
	{
	 if ( is_name( arg2, CURRENCY_NAMES ) && is_number(arg1) )
    {
         if ((cont = get_obj_here(ch, arg3)) == NULL || !can_see_obj(ch, cont) )
         {
             act("There is no $T in $p", ch, cont, arg2, TO_CHAR);
             return;
         }
       switch ( cont->item_type )
       {
         ...
         sprintf( moneybuf, "%s %s", arg1, arg2 );
          get_ok = get_money_obj( ch, moneybuf, cont );
           if ( get_ok )
           return;
	 }
	}
    /* 'get <money> <item>' */
	 if ( is_name( arg1, CURRENCY_NAMES ) && arg2[0] != '\0' )
    {
         if ((cont = get_obj_here(ch, arg2)) == NULL || !can_see_obj(ch, cont) )
         {
             act("There is no $T in $p", ch, cont, arg1, TO_CHAR);
             return;
         }
       switch ( cont->item_type )
       {
         ...
         sprintf( moneybuf, "all");
          get_ok = get_money_obj( ch, moneybuf, cont );
           if ( get_ok )
           return;
	 }

    	/* Get x gold/silver/sun */
   ...
   if (is_name(arg1, CURRENCY_NAMES ) && arg2[0] == '\0' )
   {
	...
	/* "Get obj" to get from room */
	if (arg2[0] == '\0')
	{
		/* "Get all" */
		if ( !str_cmp("all", arg1) || !str_prefix("all.", arg1) )
		{
            if (!str_cmp("all", arg1) ) /* doot */
			{
			  ...
			  for ( obj = ch->in_room->first_content; obj != NULL; obj = obj_next )
				{
				   obj_next = obj->next_in_room;
				   	if ( obj->wear_loc != WEAR_NONE )
					continue;
                   if (can_see_obj(ch, obj) && is_name(&arg1[4], obj->name))
				   {
					   get_obj(ch, obj, NULL);
					   found = TRUE;
				   }
...
/* Getting from room */
	 ...
	 {
		 act("There is no $T here.", ch, NULL, arg1, TO_CHAR);
		 return;
	 }
	 if(can_see_obj(ch, obj))
		 get_obj(ch, obj, NULL);
   ...
   /* Geting from a container */
   cont = get_obj_here(ch, arg2);

   if (cont == NULL || !can_see_obj(ch, cont))
   {
	...
	{
	    sprintf(moneybuf, "all");
        found = get_money_obj( ch, moneybuf, cont );

		if (!str_cmp("all", arg1) )
		{
    for ( obj = cont->first_in_carry_list; obj != NULL; obj = obj_next )
          {
   ...
   if (!found)
       act("I see nothing in $p.", ch, cont, NULL, TO_CHAR);
		} else
		{
    for ( obj = cont->first_in_carry_list; obj != NULL; obj = obj_next )
          {
   ...
   if (!found)
   {
	   sprintf(buf, "I see no %s in $p", arg1);
	   act( buf, ch, cont, NULL, TO_CHAR);
   }
		}
   return;
	} else { /* "Get obj container" */
...
void do_put( CHAR_DATA *ch, char *argument )
{
	...
	cont = get_obj_here(ch, arg2);
	if (obj == NULL && str_cmp("all", arg1) && str_prefix("all.", arg1))
	{
		act("You don't have any $T.", ch, NULL, arg1, TO_CHAR);
		return;
	}

	if (cont == NULL)
	{
		act("You don't see $T here.", ch, NULL, arg2, TO_CHAR);
		return;
	}
	switch ( cont->item_type ) {
	default:
	...
	if (obj == cont)
	{
		send_to_char("You can't put an object into it self.\n\r", ch);
		return;
	}
/* This is REALLY messy. Nest o rama. */
	...
	if (!str_cmp("all", arg1) || !str_prefix("all.", arg1 ) )
	{
			...
			for ( obj = ch->first_carry; obj != NULL; obj = obj_next )
			{
				...
				if(can_see_obj(ch, obj) )
				{
				 if (!can_drop_obj(ch, obj) )
                  ...
                  if ( get_obj_weight( obj ) + get_obj_weight( cont )
						> cont->value[0] )
				  {
                       act( "$p won't fit into $P.", ch, obj, cont, TO_CHAR );
					   attempt = TRUE;
                       continue;
				  }
                 if ( cont->item_type == ITEM_SPELL_MATRIX )
				 {
                    if ( obj->item_type != ITEM_ENCHANTMENT )
					{
						for (cont_obj = cont->first_in_carry_list; cont_obj != NULL; cont_obj = cont_obj->next_in_carry_list)
						{
                           if(cont_obj->item_type != ITEM_ENCHANTMENT)
						   {
							   has_item = TRUE;
							   break;
						   }
						...
						if (has_item)
						{
							send_to_char("A spell matrix can only hold 1 item that isn't a gem.\n\r", ch);
							return;
						}
					}
				 }
					...
					obj_from_char(obj);
					obj_to_obj(obj, cont);
					act("You put $p into $P.", ch, obj, cont, TO_CHAR);
					act("$N puts $p into $P.", ch, obj, cont, TO_ROOM);
				}
			}
			if (!found && !attempt)
				act("You aren't carring anything!", ch, NULL, NULL, TO_CHAR);
		}
		if (!str_prefix("all.", arg1) )
			...
			if (!found)
				act("You aren't carring any $T", ch, NULL, &arg1[4], TO_CHAR);
		}
		return;
	}
				...
				if(can_see_obj(ch, obj) )
				{
					...
					if (obj->item_type == ITEM_CONTAINER)
					{
						send_to_char("That would be a bad bad idea...\n\r", ch);
						return;
					}
...
void do_drop( CHAR_DATA *ch, char *argument )
{
	...
	if (is_number(arg1) && arg2[0] == '\0')
	{
		send_to_char("Drop how much of what type of money?\n\r", ch);
		return;
	}
/* droping money */
	if (is_number(arg1))
	  if (is_name(arg2, CURRENCY_NAMES ))
	  {
		sprintf( moneybuf, "%s %s", arg1, arg2 );
	  ...
	  if(!str_cmp("all", arg1 ) || !str_prefix("all.", arg1) )
	  {
		  if (!str_cmp("all", arg1) )
		  {
            for ( obj = ch->first_carry; obj != NULL; obj = obj_next )
			{
				...
				if ( obj->wear_loc != WEAR_NONE )
					continue;
				if (can_see_obj(ch, obj) && can_drop_obj(ch, obj) )
				{
					found = TRUE;
					obj_from_char( obj );
        	        obj_to_room( obj, ch->in_room );
				...
				if ( IS_SET( obj->extra_flags, ITEM_TRIG_DESTROY ) )
				{
                  act( "$p goes *POOF*.", ch, obj, NULL, TO_ROOM);
                  act( "$p goes *POOF*.", ch, obj, NULL, TO_CHAR);
                  extract_obj( obj );
				}
				}
			}
		  ...
		  if (!str_prefix("all.", arg1) )
		  {
		    for ( obj = ch->first_carry; obj != NULL; obj = obj_next )
			{
				obj_next = obj->next_in_carry_list;
				  if ( obj->wear_loc != WEAR_NONE )
					continue;
				if (can_see_obj(ch, obj) && can_drop_obj(ch, obj) && is_name(&arg1[4], obj->name) )
				{
					found = TRUE;
					obj_from_char( obj );
        	        obj_to_room( obj, ch->in_room );
...
/* Giving money! */
	...
	{
		send_to_char("Give how much {dsilver{x or {Ygold{x to whom?\n\r", ch);
		return;
	}
	...
	{
		send_to_char("They can't see it.\n\r", ch);
		return;
	}
...
/* Stupid ACK!MUD default give, bleh
void do_give( CHAR_DATA *ch, char *argument )
{
  char victim_name[MSL];
  char container_name[MSL];
...
**** Interesting LOTS teaser here...this is how I run my individual quest system ****
        if (  ( IS_NPC( victim ) )
           && ( str_cmp( victim->pIndexData->guilds, "" ) ) )
        {
          char      questbuf[MSL];
      
          sprintf( questbuf, "%s%s%d", ch->name, victim->pIndexData->guilds, 2 );
          if (  ( get_quest( questbuf ) != NULL )   
             && ( get_iquest( ch, questbuf ) != NULL )  )
          {
...
void do_drink( CHAR_DATA *ch, char *argument )
{
	...
	if ( liquid == LIQUID_BLOOD && IS_VAMP(ch) && !IS_NPC(ch) )
	{
	   send_to_char( "{RAhh, only if it was fresh, would taste much better!{x\n\r", ch );
	   gain_bloodlust( ch, 2 );
	}
...
bool can_wear_at(CHAR_DATA * ch, OBJ_DATA * obj, int location)
{
 ...
 switch(location)
 {
  ...
  case WEAR_LIGHT:   	loc_flag=ITEM_WEAR_HOLD_HAND;		break;
  case WEAR_FINGER_L:	
  ...
  case WEAR_TAIL:	loc_flag=ITEM_WEAR_TAIL;	break;
  ...
  case WEAR_HORNS:	loc_flag=ITEM_WEAR_HORNS;	break;
  case WEAR_BEAK:	loc_flag=ITEM_WEAR_BEAK;	break;
  case WEAR_FACE:	loc_flag=ITEM_WEAR_FACE;	break;
  case WEAR_EAR_L:
  case WEAR_EAR_R:	loc_flag=ITEM_WEAR_EAR;         break;
  case WEAR_HOOVES:	loc_flag=ITEM_WEAR_HOOVES;	break;
  case WEAR_AURA:       loc_flag=ITEM_WEAR_AURA;        break;
  case WEAR_HALO:       loc_flag=ITEM_WEAR_HALO;        break;
  case WEAR_WINGS:      loc_flag=ITEM_WEAR_WINGS;       break;
  case WEAR_SHOULDERS:  loc_flag=ITEM_WEAR_SHOULDERS;   break;
  case WEAR_CLAWS:      loc_flag=ITEM_WEAR_CLAWS;       break;
 ...
 if ( race_table[URANGE( 0, ch->race, MAX_RACE )].wear_locs[location] == FALSE ) 
 {
   ...
   act( "$L$n fumbles about with $p, looking for a place to wear it.",
     ch, obj, NULL, TO_ROOM );
   return FALSE;
 }
...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace )
{
    ...
    {
      sprintf( buf, "You must be level %d in a remort race to use this object.\n\r",
        obj->level );
      send_to_char( buf, ch );
      act( "$n tries to use $p, but is too inexperienced.",
        ch, obj, NULL, TO_ROOM );
    ...
    if (  ( CAN_WEAR( obj, ITEM_WEAR_FINGER ) )
       && ( can_wear_at( ch, obj, WEAR_FINGER_L ) )  )
    {
    ...
    if (  ( CAN_WEAR( obj, ITEM_WEAR_NECK ) )
       && ( can_wear_at( ch, obj, WEAR_NECK_1 ) )  )
    {
    ...
    if (  ( CAN_WEAR( obj, ITEM_WEAR_WRIST ) )
       && ( can_wear_at( ch, obj, WEAR_WRIST_L ) )  )
    {
	...
	if (    !fReplace  
	     ...
	     && ( IS_NPC( ch ) 
	        ...
	        : ch->pcdata->learned[gsn_dualwield].level > 0 )
	   )
	{
...
*/
    ...
    if (  ( CAN_WEAR( obj, ITEM_WEAR_HALO ) )
       && ( can_wear_at( ch, obj, WEAR_HALO ) )  )
    {
    ...
    if (  ( CAN_WEAR( obj, ITEM_WEAR_BEAK ) )
       && ( can_wear_at( ch, obj, WEAR_BEAK ) )  )
    {
    ...
    if (  ( CAN_WEAR( obj, ITEM_WEAR_CLAWS ) )
       && ( can_wear_at( ch, obj, WEAR_CLAWS ) )  )
    {
    ...
    if (  ( CAN_WEAR( obj, ITEM_WEAR_HOOVES ) )
       && ( can_wear_at( ch, obj, WEAR_HOOVES ) )  )
    {
	...
	if ( get_eq_char( ch, WEAR_HOLD_HAND_L ) == NULL )
	{
      if (  ( IS_WEAPON( obj ) )
         ...
         && (  ( IS_NPC( ch ) )
            || ( ch->pcdata->learned[gsn_dualwield] < 10 ) )  )
      {
     ...
     if ( worn == NULL && location == 29 )
       continue;
        ...
        if ( ( race_table[ch->race].wear_locs[location] == TRUE ) )
        {
          ...
          {
            sprintf( colbuf, "%s", "{d" );
            sprintf( eqbuf, "%s", "{dNothing{x" );
          }
          sprintf( catbuf, "%s%25s{x %-*s\n\r", colbuf, where_name[location],
...
if ( IS_SET(obj->extra_flags,ITEM_LODGED) )
    {
      send_to_char( "You have to DISLODGE this item!\n\r", ch);
      return;
    }
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
    ...
    {
	act( "{x$n offers $mself to {R" sacgodname "{x, who graciously declines.",
	    ch, NULL, NULL, TO_ROOM );
	send_to_char(
	    "{R" sacgodname "{x appreciates your offer and may accept it later.", ch );
	return;
    }
    ...
    if ( change_align )
    {
      ...
      sprintf( buf,"{C" goodgodname "{x gives you %s for your sacrifice.\n\r",
        cost_to_money( gp ) );
      send_to_char( buf, ch );
      act( "{x$n sacrifices $p to {C" goodgodname "{x.", ch, obj, NULL, TO_ROOM );
      ...
      sprintf( buf,"{R" evilgodname "{x gives you %s for your sacrifice.\n\r", 
        cost_to_money( gp ) );
      send_to_char( buf, ch );
      act( "{x$n sacrifices $p to {R" evilgodname "{x.", ch, obj, NULL, TO_ROOM );
      ...
      sprintf( buf,"{B" neutralgodname "{x gives you %s for your sacrifice.\n\r", 
        cost_to_money( gp ) );
      send_to_char( buf, ch );
      act( "{x$n sacrifices $p to {B" neutralgodname "{x.", ch, obj, NULL, TO_ROOM );
    ...
    if ( !change_align )
    {
      sprintf( buf,"{x" sacgodname "{x gives you %s for your sacrifice.\n\r", 
        cost_to_money( gp ) );
      send_to_char( buf, ch );
      act( "{x$n sacrifices $p to {x" sacgodname "{x.", ch, obj, NULL, TO_ROOM );
    }
...
void do_brandish( CHAR_DATA *ch, char *argument )
{
    ...
    if ( (  ( staff = get_eq_char( ch, WEAR_HOLD_HAND_L ) ) == NULL )
         || ( staff->item_type != ITEM_STAFF )   )
          if ( (  ( staff = get_eq_char( ch, WEAR_HOLD_HAND_R ) ) == NULL )
             || ( staff->item_type != ITEM_STAFF )   )
    {
...
void do_zap( CHAR_DATA *ch, char *argument )
{
    ...
    if ( (  ( wand = get_eq_char( ch, WEAR_HOLD_HAND_L ) ) == NULL )
         || ( wand->item_type != ITEM_WAND )   )
          if ( (  ( wand = get_eq_char( ch, WEAR_HOLD_HAND_R ) ) == NULL )
             || ( wand->item_type != ITEM_WAND )   )
    {
...
void do_steal( CHAR_DATA *ch, char *argument )
{
    ...
    if (  arg1[0] == '\0' || (arg2[0] == '\0' && (victim = get_char_room( ch, arg1 )) == NULL) )
    {
    ...
    if ( ( victim->level < (ch->level - 10) ||  victim->level > (ch->level + 10) ) && !IS_IMMORTAL(ch) )
    {
        send_to_char( "Find a mob around your level to steal from.\n\r", ch);
        return;
    }
    WAIT_STATE( ch, skill_table[gsn_steal].beats );
    if ( (rindex = get_obj_index(victim->steal_vnum) ) == NULL)
    {
        send_to_char( "You failed.\n\r", ch);
        return;
    }
    ...
    {
        send_to_char( "Bingo, you got a random item!\n\r", ch);
        rand = create_object( rindex, rindex->level );
        obj_to_char(rand, ch);
        victim->steal_vnum = 0;
        return;
    }
...
void do_list( CHAR_DATA *ch, char *argument )
{
  ...
  if ( IS_SET(ch->in_room->room_flags, ROOM_PET_SHOP) )
  {
    ...
    for ( pet = pRoomIndexNext->first_person; pet; pet = pet->next_in_room )
    {
        ...
        {
          found = TRUE;
          safe_strcat( MAX_STRING_LENGTH,  buf1, "\n\r{w[{YLevel{w]       {YPet{w                           {YPrice  ( Approximate ){x \n\r" );
        }
        stopcounter++;
        rounded_cost = round_money_off( 10 * pet->level * pet->level, 1 );
        sprintf( costbuf, "%s", money_string( rounded_cost ));
        sprintf( buf, "[ {W%3d{w]  {c%-*s{w  {W%-*s{x \n\r",
         pet->level, ccode_len(pet->short_descr, 30 ), capitalize( pet->short_descr ),
          ccode_len( costbuf, 35 ), costbuf  );
	...
	send_to_char( buf1, ch );
  send_to_char( "{mMagenta{x Level numbers indicate remort items.\n\rYou may also type list <keyword>.\n\r", ch );
	return;
    }
}
...
void do_sell( CHAR_DATA *ch, char *argument )
{
  ...
  join_money( transfer, ch->money );
    ...
    good_trade =  TRUE;
    sprintf( log_buf, "Selling for %s, Trade %s", 
    moneybuf,
    good_trade ? "{CAccepted{x" :
    "{RRejected{x" );
...

{
    char arg[MAX_INPUT_LENGTH];
    OBJ_DATA *obj;
    int place_to_put_it = hometown_table[ch->hometown].mdonate;
    ...
    if ( obj->item_type == ITEM_WEAPON )
       place_to_put_it = hometown_table[ch->hometown].wdonate;

    if ( obj->item_type == ITEM_ARMOR )
       place_to_put_it = hometown_table[ch->hometown].adonate;
...

{
    ...
    place_to_put_it = get_clan_numin(ch->pcdata->clan, CDONA);
    if ( ( room = get_room_index( place_to_put_it ) ) == NULL )
    {
...
void do_bid( CHAR_DATA *ch, char *argument )
{
   ...
   if (  ( amount > money_value( ch->money ) )
      || ( money_to_value( ch, argument ) == -1 ) )
...
void do_auction( CHAR_DATA *ch, char *argument )
{
  ...
  if ( auction_item != NULL )
  {
    ...
    bool	good_buyer = FALSE;
     ...
     sprintf( buf, "The auction of %s has been stopped by an {mImmortal{x.\n\r",
   ...
   if ( auction_item != NULL )
   {
      ...
      sprintf( buf, "The estimated value is %s, and last bid was for %s.\n\r",
         cost_to_money((int)auction_item->cost),
         lastbidbuf);
      send_to_char( buf, ch );
      sprintf( buf, "The reserve price is {W%s {x.\n\r", 
...
void do_deposit(CHAR_DATA *ch, char *argument)
{
	...
	if (arg1[0] == '\0')
	{
		send_to_char("Deposit what?\n\rSyntax: Deposit <item>\n\r -or- \n\rSyntax: Deposit <ammount> <money unit>\n\r", ch);
		return;
	}

	if ( !IS_SET(ch->in_room->room_flags, ROOM_BANK) && !IS_IMMORTAL(ch))
	{
		send_to_char("You need to be in a bank to do this!\n\r", ch);
		return;
	}
	if (is_number(arg1) )
	{
		sprintf(buf, "%s %s", arg1, arg2);
		deposit_money(ch, buf);
		return;
	}
...
void do_balance(CHAR_DATA *ch, char *argument)
{
    ...
    send_to_char("Items currentlly in storage:\n\r", ch);
...
void do_withdraw(CHAR_DATA *ch, char *argument)
{
	...
	if (arg1[0] == '\0')
	{
		send_to_char("Withdraw what?\n\rSyntax: Withdraw <item>\n\r -or- \n\rSyntax: Withdraw <ammount> <money unit>\n\r", ch);
		return;
	}
	...
	{
		sprintf(buf, "%s %s", arg1, arg2);
		withdraw_money(ch, buf);
		return;
	}

	obj = get_obj_stored(ch, arg1);

	if (obj == NULL || !can_see_sobj(ch, obj))
	{
		sprintf(buf, "You don't have any %s in storage.\n\r", arg1);
		send_to_char(buf, ch);
		return;
	}
    /* Poof, magiclly back in your inv. */
	obj->stored = FALSE;

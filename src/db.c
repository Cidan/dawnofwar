...
#include <errno.h>
#include <signal.h>
#include "special.h" /* Special loading for certain mobs */
...
int                     top_shop;
int                     top_clan;
unsigned long int       top_pco = 0;
/*
 * MOBprogram locals
*/

int             mprog_name_to_type      args( ( char* name ) );
void		mprog_file_read		args( ( char *f,
...
void boot_db( bool fCopyOver )
{
    ...
    send_to_descrips( "Initializing DoW.  Please Wait....\n\r" );
...

    {
      ...
      for (index = SUPER_NONE; index < MAX_SUPER; index++ )
      {
        ...
        switch ( index )
        {
        case SUPER_NONE : sprintf( buf, "%s", "ORDINARIES" );
	 			 break;
        case SUPER_VAMP : sprintf( buf, "%s", "{GJUSTICARS{x" );
                          break;

        default: sprintf( buf, "%s", "NONE" );
                 break;
        }
 ...
 */
	...
	for ( ; ; )
	{
		...
		else if ( !str_cmp( word, "OBJFUNS"  ) ) load_objfuns (fpArea);
		else
		{
...
//	load_notes( );
  ...
  load_corpses( );
  log_f( "Loading Hidden Items" );
  load_hobj( );
  log_f( "Loading clan tables." );
  load_clans( );
  booting_up = TRUE;
...
/* MAG Mod for optionals additions to area headers. */
    ...
    {    
     ...
     {
      case 'D':
       pArea->linked_main = fread_number(fp);
       break;
      case 'F':
...
void load_clans( void )
{
  ...
  sprintf( corpse_file_name, "%s", NEW_CLAN_FILE );
...
void load_mobiles( FILE *fp )
{
	...
	pMobIndex->hp                   = fread_number( fp ); 
	pMobIndex->mp                   = fread_number( fp ); 
    pMobIndex->hometown             = fread_number( fp );
  ...
  if ( letter == '|' )
  {
     ...
     pMobIndex->report_room = fread_number( fp );
     pMobIndex->chance_of_item = fread_number( fp );
	}
	else 
	   ungetc( letter, fp );
...
void refresh_this_mobile(int vnum)
{
	return;
}
...
void load_objects( FILE *fp )
{
  ...
  if ( area_revision < 15 )
  {
    for ( looper = 4;
    looper < 10;
     pObjIndex->value[looper] = 0, looper++ );
  }
  else
  {
    for ( looper = 4;
	...
	for ( ; ; )
	{
		...
		paf->bitvector          = 0;
		paf->caster		= NULL;
		LINK(paf, pObjIndex->first_apply, pObjIndex->last_apply,
		     next, prev);
		top_affect++;
	    }
		...
		LINK(ed, pObjIndex->first_exdesc, pObjIndex->last_exdesc,
		     next, prev);
		top_ed++;
	    }
...
void load_resets( FILE *fp )
{
	...
	case 'A': reset_ok = FALSE;  break; /* obsolete reset, discard */
	case 'M':
	...
	if ( reset_ok )         /* i.e. only add if reset is valid. */
	{
	   ...
	   LINK(pReset, area_load->first_reset, area_load->last_reset,
	        next, prev);
	   top_reset++;
	   GET_FREE(pList, build_free);
	   pList->data = pReset;
...
void load_rooms( FILE *fp )
{
  ...
  {
    char testbuf[MSL];
    sprintf( testbuf, "loading rooms, vnum is %d", 
      pRoomIndex->vnum );
  ...
  pRoomIndex->treasure = room_treasure;
	...
	for ( ; ; )
	{
        ...
        pexit->is_named         = fread_number( fp );
        pexit->seperate_plane   = fread_number( fp );
		pexit->exit_info        = 0;
		...
		ed->keyword             = fread_string( fp );
		ed->description         = fread_string( fp );
		LINK(ed, pRoomIndex->first_exdesc, pRoomIndex->last_exdesc,
		     next, prev);
		top_ed++;
	    }
	    else
...
/* MAG Mod */
	GET_FREE(pList, build_free);
	pList->data     = pShop;
	LINK(pList, area_load->first_area_shop, area_load->last_area_shop,
	     next, prev);
...
void load_specials( FILE *fp )
{
	...
	case 'M':
	    ...
	    else
	    {
	       ...
	       GET_FREE(pList, build_free);
	       pList->data = pMobIndex;
	       LINK(pList, area_load->first_area_specfunc,
	            area_load->last_area_specfunc, next, prev);
            }
	    break;
	}
...
void load_objfuns( FILE *fp )
{
  for ( ; ; )
  {
	   ...
	   if ( ( pObj = get_obj_index( fread_number( fp ) ) ) == NULL )
     {
       ...
       char buf[MSL];
       temp = fread_word( fp );
       sprintf( buf, "Error in Load Objfuns:  area %s has Objfun without corresponding object.  Save this area after booting complete to remove.", strArea );
       log_f( buf );
       free_string( temp );
     }
     else
	   ...
	   if ( pObj != NULL )
     {
       if ( pObj->obj_fun == 0 )
       {
	     ...
	     else
	     {
	       ...
	       GET_FREE(pList, build_free);
	       pList->data = pObj;
	       LINK(pList, area_load->first_area_objfunc, 
	           area_load->last_area_objfunc, next, prev);
	     }
	   }
	   break;
    	      ...
    	      pRoomIndex  = pRoomIndex->next )
    	{
          ...
          if ( ( pexit = pRoomIndex->exit[door] ) != NULL )
          {
               ...
               || ( get_room_index( pexit->vnum ) == NULL )  )
                    {
                      sprintf( buf, "Bad exit vnum %d in room %d", pexit->vnum,
                        pRoomIndex->vnum);
                      bug( buf, 0 );
  ...
  for ( pArea = first_area; pArea; pArea = pArea->next )
  {
    ...
    for ( pReset = pArea->first_reset; pReset; pReset = nextReset )
    {
      ...
      if ( ValReset != VALID_RESET )
      {
        ...
        switch(ValReset)
        {
          case INVAL_ROOM: sprintf(buf, "Invalid room for reset: "); break;
          case INVAL_OBJ:  sprintf(buf, "Invalid obj  for reset: "); break;
          case INVAL_MOB:  sprintf(buf, "Invalid mob  for reset: "); break;
          case INVAL_GEN:
          default:         sprintf(buf, "Invalid arg  for reset: "); break;
        }
        SHOW_AREA;
        ...
        bug(buf, 0);
        {
          ...
          for( guilty_reset = reset_room->first_room_reset;
               guilty_reset;
          ...
          { 
            UNLINK( guilty_reset, reset_room->first_room_reset, 
              reset_room->last_room_reset, next, prev );
...
/********************************************     
   No god forsaken Reset messages!
*********************************************
...
void reset_area( AREA_DATA *pArea )
{
	...
	case 'O':
      ...
      else if (  ( IS_SET( pRoomIndex->room_flags, ROOM_NO_REPOP ) )
         && ( pRoomIndex->first_person != NULL )  )
      {
        extract_obj( obj );
      }
...
/*  mob->move_to	= NO_VNUM; */
	...
	/*
    mob->max_hit        = mob->level * 15 + number_range(
				mob->level * mob->level / 2,
				mob->level * mob->level / 1 );
    mob->hit            = mob->max_hit;

	*/
	mob->max_hit        = pMobIndex->hp;
	mob->hit            = mob->max_hit;
    mob->max_mana       = pMobIndex->mp;
    ...
    mob->mana           = level * 25;
    */
    /* Random Item's able to be stolen.
     */
    if ( number_percent() <= pMobIndex->chance_of_item )
    {
        /* Pick an obj, any obj */
                                /* Make sure it's a holdable item. */
        while (obj == NULL || !IS_SET(obj->wear_flags, BIT_24) )
         obj = get_obj_index( number_range(1, 65535) );

            mob->steal_vnum = obj->vnum;
    } else
    {             
        mob->steal_vnum = 0;
    }
    ...
    if (mob->spec_fun != spec_thant_scout)
        mob->report_room = 0;
    else
        mob->report_room = pMobIndex->report_room;
    ...
    mob->ngroup = NULL;
    mob->spotted = NULL;
    mob->was_in_room_guard = NULL;
    mob->going_to_town = FALSE;
    mob->hunt_dest = NULL;
    mob->going_to_post = FALSE;

  GET_FREE( money, money_type_free );
#ifdef DEBUG_MONEY
  {
    char testbuf[MSL];
    sprintf( testbuf, "create_mobile, %s", 
      mob->name );
...
OBJ_DATA *create_object( OBJ_INDEX_DATA *pObjIndex, int level )
{
    ...
    OBJ_INDEX_DATA *random_index = NULL;
    AFFECT_DATA *af;
    AFFECT_DATA *new_af;
    int  new_cost;
    int looper;
  ...
  {
    char testbuf[MSL];
    sprintf( testbuf, "create_object, %s", 
      obj->name );
    money->money_key = str_dup( testbuf );
  }
#endif
...
*/
    ...
    if ( pObjIndex->value[7] > 0 )
      obj->timer = pObjIndex->value[7];
        ...
        if (pObjIndex->item_type == ITEM_RANDOM_CHEST)
        {
           if (number_percent() < pObjIndex->value[0])
           {
            obj->trapped = TRUE;
            how_many = number_range(0, 3);
            for (num = 0; num < MAX_CURRENCY; num++)
                obj->money->cash_unit[num] = number_range(0, 500);
            while (how_many > 0)
            {
                while (random_obj == NULL )
                {
                    random_index = get_obj_index(number_range(1, 65535) );
                    if (random_index != NULL
                        && IS_SET(random_index->wear_flags, BIT_24 )
                        && random_index->item_type != ITEM_RANDOM_CHEST )
                   {
                    random_obj = create_object(random_index, random_index->level);
                    obj_to_obj(random_obj, obj);
                   }
                }
                random_obj = NULL;
                how_many--;
        ...
        SET_BIT(obj->value[1], CONT_CLOSEABLE);
...
void do_areas( CHAR_DATA *ch, char *argument )
{
	...
	if (  ( !fall )
           && (  ( pArea->min_level > ( get_psuedo_level( ch ) ) )
...
int number2_bits(int width)
{
    return number2_mm() & ((1 << width) - 1);
}

long number2_mm(void)
{
    return random() >> 6;
}
...
/*
  char truename[MSL];
  one_argument(pMobIndex->name, truename);
  sprintf(f, "%d-%s",pMobIndex->vnum, truename);
 */

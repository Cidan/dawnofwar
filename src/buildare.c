...
void build_makearea( CHAR_DATA *ch, char *argument )
{
  ...
  rooms=atoi(arg2);
   if (rooms < 3 || rooms > 1000 )
     {
	
	send_to_char("Rooms between 3 and 1000\n\r",ch);
	sprintf(buf,"%d\r\n",rooms);
	send_to_char(buf,ch);
	return;
...
void build_setarea( CHAR_DATA *ch, char * argument)
{
    ...
    if ( !str_cmp( arg1, "message"))
    {
     if (!str_cmp( arg2, "NONE" ) )
      {
	build_strdup(&pArea->reset_msg,"",TRUE,ch);
	return;
      } else {
...
void build_arealist( CHAR_DATA *ch, char *argument )
{
  ...
  for ( pArea = first_area; pArea != NULL; pArea = pArea->next )
  {
      sprintf( buf, "%12s [%5d to %5d] %s\n\r", capitalize( pArea->owner ),
               pArea->min_vnum, pArea->max_vnum, pArea->name );
      stop_counter++;
      if (stop_counter > 40 )
      {

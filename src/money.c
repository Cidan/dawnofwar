...
{
  { "silver", "{dsilver{x", "{dsilver{x", .01, 1 },
  { "gold", "{Ygold{x", "{Ygold{x", .05, 20 },
};
...
bool give_money( CHAR_DATA * ch, CHAR_DATA * victim, char * argument )
{
  ...
  sprintf(mbuf, "You give %s to $N.", money_string( transfer ) );
  act(mbuf, ch, NULL, victim, TO_CHAR);
  sprintf(mbuf, "$n gives you %s.", money_string( transfer ) );
  act(mbuf, ch, NULL, victim, TO_VICT);
...
bool withdraw_money( CHAR_DATA * ch, char * argument )
{
  ...
  sprintf( outbuf, "You withdraw %s %s.\n\r", m_number, m_name );
  send_to_char( outbuf, ch );
...
void deposit_money( CHAR_DATA * ch, char * argument )
{
  ...
  for( ; ; )
  {
    ...
    sprintf( outbuf, "You deposit %s %s.\n\r", m_number, m_name );
    send_to_char( outbuf, ch );
    ch->money->cash_unit[mn] -= atoi( m_number );
    transfer->cash_unit[mn] += atoi( m_number );
  }
...
void drop_money( CHAR_DATA * ch, char * argument )
{
  ...
  sprintf( mbuf, "$n drops %s.", money_string( transfer ) );
  act( mbuf, ch, NULL, NULL, TO_ROOM );
  sprintf( mbuf, "You drop %s.", money_string( transfer ) );
  act( mbuf, ch, NULL, NULL, TO_CHAR );
...
/*    char arg2[MAX_INPUT_LENGTH];  */
   ...
   if ( argument[0] == '\0' )
   {
      ...
      send_to_char( "Use FULL names when handling money. Examples below.\n\r\n\r", ch );
      send_to_char( "Bank deposit 2 silver.\n\r", ch );
      send_to_char( "Bank withdraw 2 silver.\n\r", ch );
      send_to_char( "Bank exchange 20 silver.\n\r\n\r", ch );
      send_to_char( "20 Silver = 1 gold.\n\r", ch );
      send_to_char( "1000 Gold = 1 Royal Sunburst.\n\r", ch );
      return;
   }

  ...
  { 
   ...
   if ( saving_area== START_SAVING)
       ...
       if (SaveFile==NULL)
	{
	 ...
	 send_to_char("{RCan not open file for saving.{x\n",CurSaveChar);
	 return;
	}
    ...
    switch (Section)
     {
       ...
       case BUILD_SEC_MOBILES:  build_save_mobs();     break;
       case BUILD_SEC_MOBPROGS: build_save_mobprogs(); break;
       case BUILD_SEC_OBJECTS:  build_save_objects();  break;
       case BUILD_SEC_SHOPS:    build_save_shops();    break;
       case BUILD_SEC_RESETS:   build_save_resets();   break;
       case BUILD_SEC_SPECIALS: build_save_specs();    break;
       case BUILD_SEC_OBJFUNS:  build_save_objfuns();  break;
...
void build_save_help()
{
	...
	return;
      ...
      send_to_char("{cSaving help section.{x\n",CurSaveChar);
...
void build_save_mobs()
{
       ...
       if (CurSaveArea->first_area_mobile==NULL)
	{
     ...
     fprintf(SaveFile,"%i %i %i\n",pMobIndex->ac_mod, pMobIndex->hr_mod,
	  ...
	  if (pMobIndex->mp <= 0)
         pMobIndex->mp = 1;
      if (pMobIndex->hometown < 0 || pMobIndex->hometown > 4)
         pMobIndex->hometown = 0;

	 fprintf(SaveFile, "%i %i %i\n", pMobIndex->hp, pMobIndex->mp, pMobIndex->hometown);
   ...
   fprintf( SaveFile, "| %i %i %i %i %i %i %i %i %i\n",
      ...
      pMobIndex->resist, 
      pMobIndex->suscept,
      pMobIndex->report_room,
      pMobIndex->chance_of_item);
     ...
     while (mprg)
      {
       if (mprg->filename == NULL)
           bug("No filename for mprog %d!", 1 );
       else
	{
      sprintf(mprgfile, "%s%s", MOB_DIR,mprg->filename);
      if ( !(fp = fopen(mprgfile, "r")) )
      {
...
void build_save_mobprogs()
{
       ...
       if (CurSaveArea->first_area_mobprog==NULL)
	{
	 ...
	 return;
       ...
       send_to_char("{cSaving mobprogs.{x\n",CurSaveChar);
     ...
     one_argument(pMobIndex->player_name, truename);
     sprintf(f, "%d-%s",pMobIndex->vnum, truename);
     free_string(pItem->filename);
     pItem->filename = str_dup(f);
...
void build_save_objects()
{
       ...
       if (CurSaveArea->first_area_object==NULL)
	{
	 ...
	 return;
       ...
       send_to_char("{cSaving objects.{x\n",CurSaveChar);
...
void build_save_rooms()
{
       ...
       if (CurSaveArea->first_area_room==NULL)
	{
	 ...
	 fprintf(SaveFile,"%s~\n",pexit->description);
	 fprintf(SaveFile,"%s~\n",pexit->keyword);
     fprintf(SaveFile,"%i %i\n", pexit->is_named, pexit->seperate_plane); /* Is it named? Is it it's own plane? */
	 /* Deal with locks */
...
void build_save_specs()
{
       ...
       if (CurSaveArea->first_area_specfunc==NULL)
	{
	 ...
	 return;
       ...
       send_to_char("{cSaving specials.{x\n",CurSaveChar);
...
void build_save_objfuns()
{
       ...
       if (CurSaveArea->first_area_objfunc==NULL)
	{
	 ...
	 return;
       ...
       send_to_char("{cSaving objfuns.{x\n",CurSaveChar);
...
void build_save_end()
{
     ...
     sprintf( buf, "{BFinished saving %s{x", CurSaveArea->filename );
     monitor_chan( buf, MONITOR_AREA_SAVING );
     
     fprintf(SaveFile,"#$\n");
     send_to_char("{BFinished saving.{x\n",CurSaveChar);
     fclose(SaveFile);
     /* Save backup */

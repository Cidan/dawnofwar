...
/* first, we check for good enchantments and an item template in the matrix */
    ...
    if ( ( matrix = get_eq_char( ch, WEAR_HOLD_HAND_R ) ) == NULL )
    {
      ...
      {
        send_to_char( "{xYou must be holding a {Bspell matrix{x for this ability!\n\r", ch );
        return;
      }
    }
    if ( matrix->item_type != ITEM_SPELL_MATRIX )
    {
      send_to_char( "{xThat is not a {Bspell matrix{x you are holding, you fool!!\n\r", ch );
      return;
    }
...
/* now, we tear apart the current item's affs and store them in cur_mod vars */
  ...
  if ( !str_cmp( arg1, "show" ) )
  {
    ...
    if ( legal_enchant )
    {
      sprintf( cat_buf, "Total Cost: %d {Cqp{x\n\r", qp_cost );
    }
    else
    {
    ...
    if ( legal_enchant )
    {
      ...
      {
        sprintf( cat_buf, "{RWARNING: {WEnchanting %s with these affects will make the item level %d, which is higher than your current ability to use.\n\r",
    ...
    do_save( ch, "" );
	  ...
	  char *strtime;
		...
		affect_loc_name( one_aff->location ), one_aff->modifier );
          ...
          brand->branded_by = str_dup( "{GSystem{x");

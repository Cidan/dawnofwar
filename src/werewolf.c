...
/*    int value, max;   */
  ...
  if ( ( arg1[0] == '\0' ) || ( ch->pcdata->generation > 1 )  )
  {
      ...
      for ( index = 0; index <= 4 ; index++ )
      {
        ...
        for ( d = first_desc; d!=NULL; d = d->next )
        {
            ...
            &&   victim->in_room != NULL )
            {
                ...
                sprintf( buf, "%-15s {xStanding: {G%s   {xRank: {d%d{x  {xRage: {R%d{x/{R%d{x   %-15s\n\r",
        ...
        if ( !found )
            send_to_char( "{xNo {yGarou{x Tribe members were found.\n\r", ch );
    ...
    else
    {
      ...
      sprintf( buf, "{WMembers of the {yGarou {GTribe %s\n\r", get_tribe_name( ch ) );
      send_to_char( buf, ch );
      found = FALSE;
      for ( d = first_desc; d!=NULL; d = d->next )
      {
        if ( d->connected == CON_PLAYING
          ...
          && !IS_IMMORTAL( victim )   )
          {
              ...
              found = TRUE;
                sprintf( buf, "%-15s {xStanding: {G%s   {xRank: {d%d{x\n\r",
       ...
       if ( !found )
          send_to_char( "{xNo other {yGarou{x {GTribe{x members were found.\n\r", ch );
  ...
  else     /* multiplexed use of tribe  */
  {
    ...
    {
      send_to_char( "They are not in your {yTribe{x!\n\r", ch );
      return;
    }
    ...
    if ( !str_cmp( arg2, "standing" ) )
    {
      ...
      {
        send_to_char( "{YStandings{x: mate warder guardian sentry elder adult younger cub.\n\r", ch );
        return;
      }
 
      sprintf( buf, "{x%s's {YStanding{x is now %s in {yTribe{x %s.\n\r", 
         victim->name,
    ...
    {
      int	sn;
       sprintf( buf, "{x%s has been {RBANISHED{x from {yTribe{x %s.\n\r", 
         victim->name,
         get_tribe_name( ch )   );
      do_howl( ch, buf );
...
void do_imprint( CHAR_DATA *ch, char *argument )
 {
    ...
    {
      send_to_char( "Your {yTribal {YStanding{x is not high enough to imprint!\n\r", ch );
      return;
    }
   ...
   if ( !IS_WOLF( victim ) )
   {
     send_to_char( "{RWHAT????{x You want to teach {yGarou{x skills to MORTAL{x? NO WAY!!", ch );
     return;
   }
...
/* okay, we know the victim is good, and a wolf..check for skill known, levels, etc. */
   ...
   {
     send_to_char( "{xYou don't know that {yGarou{x skill!.\n\r", ch );
     return;
   }
...
/* FIX ME
   ...
   {
     send_to_char("{xThey are too inexperienced in the ways of the {yGarou{x to learn this skill.\n\r", ch );
     send_to_char( "{xYou are to inexperienced in the ways of the {yGarou{x to learn this skill.\n\r", victim );
     return;
   }
*/
   if ( victim->pcdata->vamp_skill_num >= victim->pcdata->vamp_skill_max )
   {
     ...
     send_to_char( "{xYou are unable to learn any more {yGarou{x knowledge at this time.\n\r", victim );
     return;
   }
...
/* Okay, the skill is good, the instructor knows it, and the victim doesn't  */
   ...
   send_to_char( "You have increased their {yGarou{x instincts{x!!!\n\r", ch );
   sprintf( buf, "You are now instinctively aware of {R%s{x!!!\n\r", skill_table[sn].name );
...
bool spell_tribe_claw(int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
    ...
    {
      send_to_char( "{dOutcasts{x can't tribal claw!!!\n\r", ch );
      return FALSE;
    }

    if ( IS_WOLF( victim ) )
     {
       send_to_char( "{xYour selection is already a {yGarou{x!!!.\n\r", ch );
       return FALSE;
     }
...
*/
    if ( IS_VAMP( victim ) )
    {
      send_to_char( "A sudden awareness comes over you, as you peer into their soul and see a {dKindred{x!!!\n\r", ch );
      if ( !IS_AWAKE( victim ) )
        do_wake( victim, "" );
     ...
     if ( vamps > 0 )
     {
       send_to_char( "In front of {dKindred{x??? Not a chance!!\n\r", ch );
       return FALSE;
     }
     else if ( mortals > 1 )
     {
       send_to_char( "In front of {CMortals{x??? Not a chance!!\n\r", ch );
       return FALSE;
     }
     else if ( wolves < 4 )
     {
       send_to_char( "You must have more {yGarou{x witnesses to the event.\n\r", ch );
       return FALSE;
     }
...

     {
       ...
       send_to_char( "{xYou thrash the cowering form beneath you, raking your {Rclaws{x along it's soul.\n\r", ch );
       send_to_char( "Then, in the final death throes, you chomp it's neck, tearing out it's essence,{x\n\r", ch );
       send_to_char( "and breathe your Tribal strength into the bloody corpse, making them forever submissive to your {ytribe{x!!!\n\r", ch );

       send_to_char( "{xYou feel your soul ripped asunder as the {yGarou{x {RClaws{x your very soul to shreds{x!!\n\r", victim );
       send_to_char( "As it tears your throat out, and you begin to submit to death itself, you suddenly feel\n\r", victim );
       send_to_char( "a strong, earthly essence overcome your soul.  As the strength of the {yGarou{x begins to fill\n\r", victim );
       send_to_char( "your veins, your mind is flooded with the sudden knowledge of {yTribal {xpower and everlasting loyalty to the {GEldest{x.\n\r", victim );
...
bool spell_wolf_mark(int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
  ...
  switch ( ch->pcdata->vamp_bloodline )
  {
    ...
    case 2 : costring = "{G"; break;
    case 3 : costring = "{R"; break;
    case 4 : costring = "{y"; break;
